...
| 6d–6e | 화면을 협업 편집기로 넘기기(오프라인 보존 포함) · 낡은 저장 경로 걷어내기 | 진행 중 |

**화면은 아직 Phase 0 편집기입니다.** 서버와 연결 단위는 만들었고, 편집기를 거기에 붙이는 것이 6d 입니다.

그다음은 코멘트와 멘션 · Board · relation · 템플릿입니다.

**숫자** — 마이그레이션 17개 · 테이블 39개 · 테스트 1,672개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 212개 · PR 86개 머지.
...
**헤드리스 테스트가 못 보는 것은 실제 브라우저로 봅니다.** ProseMirror는 DOM 없이 돌아 커맨드 · 입력 규칙이 `node --test`로 검증되지만, 브라우저에서만 드러나는 버그 넷이 그 테스트를 전부 통과한 채 들어가 있었습니다. [`e2e-editor.mjs`](scripts/e2e-editor.mjs)가 Chrome DevTools Protocol로 실제 입력을 넣어 212개 항목을 확인합니다.
