...
| 5c | rollup — 읽을 때 계산 · 볼 수 없는 행은 집계에서 제외 | ⬜ |

**숫자** — 마이그레이션 24개 · 테이블 50개 · 테스트 1,806개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 344개 · PR 105개 머지.
...
**헤드리스 테스트가 못 보는 것은 실제 브라우저로 봅니다.** ProseMirror는 DOM 없이 돌아 커맨드 · 입력 규칙이 `node --test`로 검증되지만, 브라우저에서만 드러나는 버그 넷이 그 테스트를 전부 통과한 채 들어가 있었습니다. [`e2e-editor.mjs`](scripts/e2e-editor.mjs)가 Chrome DevTools Protocol로 실제 입력을 넣어 344개 항목을 확인합니다 — **앱과 협업 서버를 함께 띄우고, 탭 두 개로 동시 편집까지** 봅니다.
...
src/lib/
  ...
  database/                      프로퍼티 · 셀 · 필터 컴파일러 · 뷰 · 그룹(보드) · 드래그 규칙 · relation
  file/                          업로드 · 스토리지 드라이버
