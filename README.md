...
- **공유** — 페이지 단위 권한, 상속과 "따로 관리하기", 즐겨찾기 · 최근 방문. 사람 대신 **그룹**에게도 줍니다 — 그룹에 들어온 사람은 곧바로 보고, 빠지면 열어 둔 편집기까지 곧바로 닫힙니다(그룹을 만드는 화면은 아직이고 API 로 만듭니다)
...
| 7b | 그룹 화면 — 워크스페이스 홈에서 그룹을 만들고 사람을 넣습니다 | ⬜ |
| 7c~ | teamspace — 사이드바의 팀 공간 · 공개 범위 · Private | ⬜ |

**숫자** — 마이그레이션 27개 · 테이블 50개 · 테스트 1,964개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 421개 · PR 116개 머지.
