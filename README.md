...
- **관계형(relation)** — 다른 표(또는 같은 표)의 행과 연결하고, 표 · 목록 · 보드 카드에서 연결된 행의 제목을 칩으로 봅니다. 양방향이면 반대쪽 표에도 보입니다. 볼 수 없는 표의 행은 제목 없이 개수만 나옵니다 *(연결은 아직 API 로 — 고르는 화면은 다음 조각)*
...
| 5b-2 | 쓰기 경로 — 행 고르기 · 속성 추가 폼의 "관계형" | ⬜ |
| 5c | rollup — 읽는 시점에 계산 | ⬜ |

**숫자** — 마이그레이션 24개 · 테이블 50개 · 테스트 1,801개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 324개 · PR 104개 머지.
...
**헤드리스 테스트가 못 보는 것은 실제 브라우저로 봅니다.** ProseMirror는 DOM 없이 돌아 커맨드 · 입력 규칙이 `node --test`로 검증되지만, 브라우저에서만 드러나는 버그 넷이 그 테스트를 전부 통과한 채 들어가 있었습니다. [`e2e-editor.mjs`](scripts/e2e-editor.mjs)가 Chrome DevTools Protocol로 실제 입력을 넣어 324개 항목을 확인합니다 — **앱과 협업 서버를 함께 띄우고, 탭 두 개로 동시 편집까지** 봅니다.
