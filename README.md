...
- **코멘트 · 알림** — 페이지에, 또는 **본문에서 고른 글자에** 코멘트를 달고 답글 · 해결 · 고치기 · 지우기. 코멘트가 달린 글자는 하이라이트로 남고, 동시 편집으로 글이 밀려도 같은 글자를 따라갑니다. 다른 사람이 달면 **인박스**로 오고(읽음 · 보관 · 안 읽음 배지), 뮤트하면 오지 않음. 볼 수 없게 된 페이지의 알림은 목록에서 빠짐
...
| 5b | `@` 자동완성 화면 · 멘션 이름 그리기 · 백링크 표시 | ⬜ |

**숫자** — 마이그레이션 21개 · 테이블 46개 · 테스트 1,683개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 256개 · PR 95개 머지.
