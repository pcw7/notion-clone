...
- **보드(칸반)** — 같은 데이터를 뷰 탭에서 표 ↔ 보드로 바꿔 봅니다. 선택 · 체크박스 값이 열이 되고, **카드를 끌어 놓으면 그 값이 실제로 바뀝니다**(셀 값과 열 안 순서를 서버가 한 번에 씁니다). 열 머리의 `+` 는 그 열의 값이 채워진 카드를 만들고, 그룹 기준 · 빈 그룹 숨김 · 그룹별 숨기기를 고를 수 있습니다. 정렬이 걸린 보드는 순서를 정렬이 정하므로 열 안 이동을 막습니다
...
| 4c | status 프로퍼티 · List 뷰 | ⬜ |

**숫자** — 마이그레이션 22개 · 테이블 49개 · 테스트 1,733개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 295개 · PR 98개 머지.
...
**헤드리스 테스트가 못 보는 것은 실제 브라우저로 봅니다.** ProseMirror는 DOM 없이 돌아 커맨드 · 입력 규칙이 `node --test`로 검증되지만, 브라우저에서만 드러나는 버그 넷이 그 테스트를 전부 통과한 채 들어가 있었습니다. [`e2e-editor.mjs`](scripts/e2e-editor.mjs)가 Chrome DevTools Protocol로 실제 입력을 넣어 295개 항목을 확인합니다 — **앱과 협업 서버를 함께 띄우고, 탭 두 개로 동시 편집까지** 봅니다.
...
db/migrations/                 raw SQL 22개. ORM 없음
