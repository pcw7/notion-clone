...
- **코멘트 · 알림** — 페이지에 코멘트를 달고 답글 · 해결 · 고치기 · 지우기. 다른 사람이 달면 **인박스**로 오고(읽음 · 보관 · 안 읽음 배지), 뮤트하면 오지 않음. 볼 수 없게 된 페이지의 알림은 목록에서 빠짐
...
| 4b | 본문 글자에 붙이기 — 고른 범위로 스레드 열기 · 하이라이트 | ⬜ |
| 5 | 멘션 — `@` 자동완성 · 역인덱스 · 멘션 알림 | ⬜ |

**숫자** — 마이그레이션 20개 · 테이블 45개 · 테스트 1,672개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 241개 · PR 93개 머지.
