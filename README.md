...
지금은 **Board · Group by**(F-04-03 · F-04-11)를 하고 있습니다. 되돌리기 비싼 것부터 — 저장 모양과 서버 명령을 먼저 굳히고 화면을 올렸습니다.
...
**숫자** — 마이그레이션 23개 · 테이블 50개 · 테스트 1,761개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 306개 · PR 100개 머지.
...
**헤드리스 테스트가 못 보는 것은 실제 브라우저로 봅니다.** ProseMirror는 DOM 없이 돌아 커맨드 · 입력 규칙이 `node --test`로 검증되지만, 브라우저에서만 드러나는 버그 넷이 그 테스트를 전부 통과한 채 들어가 있었습니다. [`e2e-editor.mjs`](scripts/e2e-editor.mjs)가 Chrome DevTools Protocol로 실제 입력을 넣어 306개 항목을 확인합니다 — **앱과 협업 서버를 함께 띄우고, 탭 두 개로 동시 편집까지** 봅니다.
...
src/app/                       화면 · API 라우트 (Next.js App Router)
  ...
  w/[workspaceId]/               사이드바 · 검색 오버레이
    ...
    db/[databaseId]/             데이터베이스 표 · 보드 · 뷰 탭 · 도구줄
    inbox/                       알림 인박스
