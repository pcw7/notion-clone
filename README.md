...
지금은 **템플릿 · 복제**(F-08-01~03)를 준비하고 있습니다 — 서브트리를 복제할 때 안쪽 참조는 사본으로, 바깥 참조는 원본으로 남기는 id 재매핑 규칙을 먼저 고정합니다.

**숫자** — 마이그레이션 25개 · 테이블 50개 · 테스트 1,866개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 360개 · PR 108개 머지.
...
**헤드리스 테스트가 못 보는 것은 실제 브라우저로 봅니다.** ProseMirror는 DOM 없이 돌아 커맨드 · 입력 규칙이 `node --test`로 검증되지만, 브라우저에서만 드러나는 버그 넷이 그 테스트를 전부 통과한 채 들어가 있었습니다. [`e2e-editor.mjs`](scripts/e2e-editor.mjs)가 Chrome DevTools Protocol로 실제 입력을 넣어 360개 항목을 확인합니다 — **앱과 협업 서버를 함께 띄우고, 탭 두 개로 동시 편집까지** 봅니다.
