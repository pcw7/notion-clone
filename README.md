...
지금은 **relation · rollup**(F-03-10 · F-03-11)을 하고 있습니다. 역시 저장 모양부터입니다.
...
| 5c-2 | rollup 화면 — 표의 칸 · 속성 추가 폼의 "롤업" | ⬜ |

**숫자** — 마이그레이션 25개 · 테이블 50개 · 테스트 1,850개(DB 포함, CI 에서 skip 0) + 실제 브라우저 검증 350개 · PR 106개 머지.
...
**헤드리스 테스트가 못 보는 것은 실제 브라우저로 봅니다.** ProseMirror는 DOM 없이 돌아 커맨드 · 입력 규칙이 `node --test`로 검증되지만, 브라우저에서만 드러나는 버그 넷이 그 테스트를 전부 통과한 채 들어가 있었습니다. [`e2e-editor.mjs`](scripts/e2e-editor.mjs)가 Chrome DevTools Protocol로 실제 입력을 넣어 350개 항목을 확인합니다 — **앱과 협업 서버를 함께 띄우고, 탭 두 개로 동시 편집까지** 봅니다.
