 ...
 *
 * 그리고 드래그의 드롭 판정도 결국 "어느 그룹의 몇 번째 자리로 가는가"로
 * 귀결된다. 그 **자리를 정하는 규칙**을 DOM 없이 먼저 고정해 두면, 드래그는
 * 좌표를 자리로 바꾸는 일만 남는다. 규칙과 히트테스트를 한 PR 에 섞으면
 * 이동이 틀렸을 때 규칙이 틀린 건지 좌표가 틀린 건지 알 수 없다.
 *
 * ──────────────────────────────────────────────────────────────────────
