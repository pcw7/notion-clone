 ...
 *   **그룹 행의 "제거"가 모든 멤버의 행을 지운다.** 그래서 종류마다 이름으로 가르고, 모르는 종류(teamspace · public
 *   — 아직 없다)는 `null` 이다 — 패널은 그 행을 읽기 전용으로 그린다. 모르는 것을 아는 것으로 바꿔 보내지 않는다.
 */
 ...
 * 모른다.
 */
export function choiceValue(principal: { readonly type: 'user' | 'group'; readonly id: string }): string {
