...
| **Q7** ✅ | **Enter 분할 / Backspace 병합 세부 규칙** (자식 귀속·타입 승계·리스트 탈출) | **실측** — 자식 있는 블록 중간에서 Enter / 이종 타입 병합 / 빈 리스트에서 Enter | §7-4에 적은 규칙 | 규칙만 바뀐다(스키마 무변경). **단 미정 상태로 구현하면 구현자마다 다르게 만든다** → W4 이전 필수<br>**[해결 · W4]** 규칙을 `src/lib/editor/block-rules.ts` 에 순수 함수로 고정했다(테스트 30여 건). §7-4 가 판결한 항목은 그대로 옮겼고, §7-4 가 "분기하라"고만 한 **두 가지를 여기서 정했다**: ① 새 블록의 위치 — *자식이 있고 펼쳐져 있으면 첫 자식, 그 외에는 형제* (기준: 새 블록이 화면에서 원본 바로 다음 줄에 오되 **자식은 절대 옮기지 않는다**) ② 접힌 앞 블록으로 자식이 이관되는 병합 — *병합하면서 앞 블록을 펼친다* (접힌 곳으로 들어간 자식은 사용자에게 삭제로 보인다). 실측으로 뒤집히면 그 파일 하나만 고친다 |
