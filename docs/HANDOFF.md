...
최종 갱신: 2026-09-12 (PR #37 머지 시점 — **W5-b 편집 확장 + 이미지 업로드 완료. 남은 것은 오프라인·오류 UX**)

---

## 1. 지금 어디까지 왔나

**Phase 0 (MVP, 6~8주) 중 W1–W4 · W5-a 완료. W5-b 는 편집 확장(선택·이동·메뉴·복붙)과 이미지 업로드까지 끝났고, 낙관적 업데이트(F-05-04)·오류 UX(F-12-16)가 남았다.**
...
| **W5-b** | **편집 확장 · 파일** | **진행 중** | #27(선택) #29(이동) #30(드래그) #31(접힘·e2e) #33(메뉴) #34(복붙) #35(파일 서버) #36(이미지 블록) #37(드롭·붙여넣기) |
| W6 | 내비게이션 · 권한 | | |
| W7 | 검색 | | |
| W8 | DB 코어 · 뷰 | | |

**동작하는 것**: 이메일 OTP 로그인 → 워크스페이스 생성 → 이메일 초대 → 수락 → 워크스페이스 진입 → 페이지 생성 → 본문 편집(12종 블록 · 분할/병합 · 중첩 · `/` 메뉴 · 마크다운 · 서식) → 자동 저장 → 하위 페이지 → 페이지 이동 → 휴지통 · 복원 · 영구 삭제 → 멀티 블록 선택(Esc · Shift+↑↓ · 일괄 삭제/변환/복제/서식) → 블록 이동(`Mod+Shift+↑↓` · `⋮⋮` 드래그) · `+` 버튼 · 토글 접기 → **블록 메뉴(변환·색·복제·링크·삭제) · 복사/붙여넣기(중첩 보존) · 이미지(올리기 · 주소 입력 · 끌어다 놓기 · 붙여넣기)**.
`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 79개 항목을 확인한다**.

**숫자**: 마이그레이션 9개 / 테이블 25개 / 테스트 748개(CI skip 0) + 브라우저 검증 79개 / PR 37개 머지.

---

## 2. 다음 작업 — W5-b 의 남은 것(오프라인 · 오류 UX)
...
**권장 순서**
1. ~~이미지 업로드~~ — **끝났다**(#35 #36 #37). 로컬 디스크 드라이버 · 5 MiB · 이미지 4종.
   R2 를 붙이는 자리는 `FileStorage` 세 함수이고, 그때 `/content` 라우트가 서명 URL 로 302 하면 된다.
   **서명 URL 을 저장하지 않는다**(FS2) — 그래서 `file` 표에 URL 컬럼이 없다. MinIO 는 금지(절대 제약 1).
2. **낙관적 업데이트 · outbox** (`F-05-04`) — **다음.** 없으면 네트워크가 끊긴 사이 친 글이 사라진다.
   지금 저장은 디바운스 1초 + 낙관적 잠금뿐이다(`body-editor.tsx`).
   ⚠ 업로드도 같은 문제를 갖는다 — 올리는 중에 끊기면 그 블록은 빈 채로 남고 재시도가 없다.
3. **오류 · 복구 UX** (`F-12-16`) — 2번과 짝이다.

그 뒤가 W6(내비게이션·권한)이다. **사이드바·휴지통의 권한 필터(§7)가 거기서 풀린다.**

**착수 전에 읽을 것**
- `docs/research/05-collaboration-sync.md` F-05-04(낙관적 업데이트 · outbox) — ⚠ 이 문서의 머리말이
  "Notion 의 push 는 op 브로드캐스트가 **아니라** 레코드 버전 알림 + pull"이라고 이전 판을 정정했다
- `docs/research/12-platform-ux.md` F-12-16(오류 · 복구 UX)
- 이미지 쪽을 다시 건드린다면: `12-platform-ux.md` F-12-09(용량 한도) ·
  `09-api-integrations.md` F-09-08(서명 URL 만료) · `01-block-editor.md` F-01-15(미디어 블록)
...
| 브라우저 검증의 빈자리 | `npm run e2e`(#31)가 79개 항목을 **실제 입력으로** 확인한다 — 핸들·드래그·선택·이동·접힘·`+`·자동 저장·복붙·이미지(업로드·드롭·붙여넣기). **IME(한글 조합)·일반 타이핑은 아직 검사가 없다** — CDP 의 `Input.imeSetComposition` 으로 조합을 흉내낼 수 있다. 드롭·붙여넣기는 합성 `DragEvent`/`ClipboardEvent` 라 **OS 수준 드래그는 재현하지 않는다**(파일 선택은 `DOM.setFileInputFiles` 로 진짜다). **CI 에는 아직 넣지 않았다** — 로컬에서 안정적으로 초록인지 먼저 쌓는다 | `scripts/e2e-editor.mjs` |
...
Phase 0 W1~W4 와 W5-a(페이지 트리)가 끝났고, W5-b 도 끝났다 —
블록 선택 · 이동(키보드·드래그) · 핸들 메뉴 · 토글 접기 · 복붙(PR #27~#34) ·
이미지 업로드(#35 파일 서버 · #36 블록·참조 카운트 · #37 드롭·붙여넣기).

다음은 낙관적 업데이트·outbox(F-05-04)와 오류·복구 UX(F-12-16)이고,
그 뒤가 W6(내비게이션·권한)이다. 지금 저장은 디바운스 1초 + 낙관적 잠금뿐이라
네트워크가 끊긴 사이 친 글이 사라진다. 업로드도 같은 구멍이 있다 —
올리는 중에 끊기면 그 블록은 빈 채로 남는다(§7).
착수 전에 docs/research/05-collaboration-sync.md 의 F-05-04 와
12-platform-ux.md 의 F-12-16 을 읽어라.
...
- `npm test` 만 돌리면 DB 테스트가 **조용히 빠진다**(카운트에도 안 잡힌다).
  `npm run db:up` 을 먼저 해야 748개가 다 돈다 — CI 의 db 잡과 같은 수여야 한다
