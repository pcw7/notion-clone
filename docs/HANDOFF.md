...
최종 갱신: 2026-09-14 (PR #65 시점 — **Phase 0 MVP 가 닫혔다. Phase 1 의 첫 항목 익스포트(F-09-14) v1 도 닫혔다** — 1 · 2a · 2b · 3a · 3b(내려받기 · 요약 라우트 · 페이지 · 표 · 워크스페이스 버튼 · e2e). **다음은 CRDT 동시편집(F-05-01) · 공개 링크(F-06-06) · 인라인 DB**, §2)
...
**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1476개(CI skip 0 · 이 PC 는 unzip 이 없어 1개 skip) + 브라우저 검증 206개(이 PC 에서 204 통과 — 클립보드 2개) / PR 64개 머지(#65 포함).

---

## 2. 다음 작업 — Phase 1

**Phase 0 이 닫혔고, Phase 1 의 최우선이던 익스포트(F-09-14) v1 도 닫혔다(#65).** 다음은
CRDT 동시편집(F-05-01) · 공개 링크(F-06-06) · 인라인 DB 다. 착수 전에 마스터 문서 §5 의 Phase 1 과
해당 도메인 문서의 F-ID 절을 읽는다. 익스포트에 남은 것(잡 · 감사 로그 · 정책 게이트 · HTML · PDF)은 §7.
...
다음 작업은 Phase 1 의 다음 항목이다 — CRDT 동시편집(F-05-01) · 공개 링크(F-06-06) · 인라인 DB.
착수 전에 마스터 문서 §5 의 Phase 1 과 해당 도메인 문서의 F-ID 절을 읽고 순서를 정한다.
  - CRDT 는 본문 순서의 정본이 Y.Doc 이라는 판결(X-1)대로 프로젝터(save-page-body.ts)의 상류를 바꾸는 일이다
  - 인라인 DB 는 블록 타입 레지스트리 + 노드 뷰와 **함께** 와야 한다(§7 — 반쯤 만들면 프로젝터가 지운다)
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1476개가 다 돌아야 CI 의 db 잡과 같다.
