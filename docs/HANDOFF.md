...
**숫자**: 마이그레이션 21개 / 테이블 46개 / 테스트 1697개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 265개(이 PC 에서 263 통과 — 클립보드 2개) / PR 96개 머지(#97 포함).
...
공개 링크(F-06-06) · 인라인 DB 는 §5.2 표에 없다. **지금은 §5.2 의 3번 — 코멘트 · 멘션 · 알림 · 인박스**를 조각으로
진행 중이다(아래 코멘트 조각 표). 익스포트에 남은 것(잡 · 감사 로그 · 정책 게이트 · HTML · PDF)은 §7.
...
지금은 마스터 문서 §5.2 의 **3번 — 코멘트 · 멘션 · 알림 · 인박스**를 조각으로 나눠 진행 중이다.
HANDOFF §2 의 **코멘트 조각 표**와 "1조각이 정한 것"을 먼저 읽어라. 그 앞의 CRDT 조각 표 · 순서 근거 ·
"N조각에서 확인한 것"은 본문을 건드릴 때 읽는다.
...
다음 작업 — 순서대로:
  ① **§5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이 닫혔다.** 다음은 4번 — **Board · Group by**(F-04-03 · F-04-11 · F-03-05 ·
     F-04-04). 마스터 문서: *"드롭 = '셀 값 + 순서' 2 mutation 을 **서버 단일 API** 로"*. 정본 §3.5(데이터베이스 · 프로퍼티 · 셀)와
     §3.6(뷰 · 레이아웃)을 먼저 읽고, W8 이 만든 표 뷰(`src/lib/database/` · `app/w/[workspaceId]/db/`)가 무엇까지 갖고
     있는지(§2-0 W8 기록 · §7 의 W8 빈자리)를 본다. 조각은 되돌리기 비싼 순서 — 저장 모양(그룹 열 · 카드 순서)부터
