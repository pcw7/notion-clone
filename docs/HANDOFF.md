...
최종 갱신: 2026-09-13 (PR #62 시점 — **Phase 0 MVP 가 닫혔다. Phase 1 익스포트(F-09-14) 진행 중 — 1(Markdown 직렬화기) · 2a(ZIP · CSV) · 2b(이름 · 조립 · 스트림) · 3a(스냅샷 읽기 · 읽기 격리) 끝. 다음은 3b(다운로드 라우트 · 페이지 메뉴 · e2e)**, §2 조각 표)
...
**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1462개(CI skip 0 · 이 PC 는 unzip 이 없어 1개 skip) + 브라우저 검증 194개(이 PC 에서 192 통과 — 클립보드 2개) / PR 61개 머지(#62 포함).
...
| 3b | 다운로드 라우트(페이지 · 워크스페이스) · 페이지 메뉴 · e2e | ⬜ |
...
✅ `withReadTransaction` 은 #62 부터 **REPEATABLE READ** 다 — 스냅샷이 서브트리 전체를 한 시점으로 읽는다
(재현 먼저, §3.3-72). 3b 의 라우트는 `readExportSnapshot` → `planExport` → `estimateExport` 로 거른 뒤 흘려보낸다.
...
Phase 0(MVP)이 닫혔다 — W1~W8 전부. Phase 1 은 익스포트(F-09-14)부터 진행 중이다.
...
익스포트는 **서버 쪽이 다 됐고 사용자가 누를 곳만 없다** — src/lib/export/:
  markdown.ts(본문 → MD) · csv.ts · zip.ts(의존성 없음) · names.ts(파일 이름) ·
  plan.ts(조립 · 보고서 · 크기 추정) · archive.ts(ZIP 스트림) ·
  snapshot.ts(권한을 SQL 에서 거르는 한 스냅샷 읽기)

다음 작업은 **익스포트 3b** 다(마스터 문서: "데이터 인질 상태는 신뢰 비용").
HANDOFF §2 조각 표와 그 아래 계획을 먼저 읽어라.
  - 다운로드 라우트(페이지 · 워크스페이스): readExportSnapshot → planExport →
    estimateExport(fits 가 아니면 흘려보내기 **전에** 거부) → streamExportZip 을 응답으로.
    첨부 바이트는 readAttachment 가 흘려보낼 때 파일 스토리지에서 읽는다.
    v1 은 동기 스트리밍이다 — 정본에 export_job 표가 없고 잡 워커도 없다(§2 계획)
  - 페이지 메뉴의 내보내기 버튼 + e2e 검사(scripts/e2e-editor.mjs)
  - 정할 것: 워크스페이스 전체 익스포트를 누가 할 수 있나(노션은 관리자만).
    정하면 §3.2 에 근거와 함께 남긴다
  - 끝나면 README 의 진행 표와 "지금 되는 것"에 익스포트를 넣는다
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1462개가 다 돌아야 CI 의 db 잡과 같다.
