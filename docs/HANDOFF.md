...
최종 갱신: 2026-09-10 (PR #27 머지 시점 — **W5-b 진행 중**)

---

## 1. 지금 어디까지 왔나

**Phase 0 (MVP, 6~8주) 중 W1–W4 · W5-a 완료. W5-b 진행 중(선택 모드까지).**
...
| **W5-b** | **편집 확장 · 파일** | **진행 중** | #27(멀티 블록 선택) |
| W6 | 내비게이션 · 권한 | | |
| W7 | 검색 | | |
| W8 | DB 코어 · 뷰 | | |

**동작하는 것**: 이메일 OTP 로그인 → 워크스페이스 생성 → 이메일 초대 → 수락 → 워크스페이스 진입 → 페이지 생성 → 본문 편집(12종 블록 · 분할/병합 · 중첩 · `/` 메뉴 · 마크다운 · 서식) → 자동 저장 → 하위 페이지 → 페이지 이동 → 휴지통 · 복원 · 영구 삭제 → **멀티 블록 선택(Esc · Shift+↑↓ · 일괄 삭제/변환/복제/서식)**.
`npm run dev` 로 실제로 눌러볼 수 있다.

**숫자**: 마이그레이션 8개 / 테이블 24개 / 테스트 545개(CI skip 0) / PR 27개 머지.
...
| **W5-b** 편집 확장 · 파일 | `F-01-08` `F-01-09` `F-01-10` `F-01-13` `F-01-15` `F-12-09` `F-09-08` `F-05-04` `F-12-16` | `F-01-09` 완료(#27), 나머지 미착수 |
...
**권장 순서** — 이 순서인 이유: **선택 모드가 없으면 이동도 복붙도 대상이 없다.**
1. ~~**멀티 블록 선택** (`F-01-09`)~~ — **완료(#27).** `src/lib/editor/block-selection.ts`.
2. **드래그·상하 이동** (`F-01-08`) — **다음.** 블록 핸들과 `Mod+Shift+↑↓`.
   대상은 `BlockSelection.rootPositions` 가 이미 준다(최상위 정규화된 상태다).
   사이드바 드래그(`F-02-03`)도 같은 조작이지만 그건 **권한 변경 트랜잭션**이라
   (F-07-16: "드래그로 섹션을 넘으면 권한이 바뀐다") W6 이후다.
3. **복붙 구조 보존** (`F-01-10`) — 단일 contenteditable 이라 "블록 경계를 넘는 부분 선택"은 이미 공짜로 된다.
   내부 복사의 슬라이스도 `BlockSelection.content()` 가 이미 돌려준다(openStart/openEnd 0).
   남은 것은 커스텀 MIME 과 붙여넣기 쪽 id 리매핑이다.
4. **이미지 업로드** (`F-01-15`) — presigned PUT. 지금은 URL 만 받는다.

**착수 전에 읽을 것**
- `docs/research/01-block-editor.md` F-01-08 / F-01-10 (특히 "블록 경계를 넘는 부분 선택"이 이미 성립한다는 점 — 단일 contenteditable 이라 공짜로 얻었다)
- `docs/research/02-page-workspace.md` F-02-13(서브페이지 vs link to page)

**F-01-08 에 미리 알아둘 것**: 블록 이동은 `blockDeletionRanges()` 가 푼 것과 같은 함정을 만난다 —
**자식을 전부 옮기면 남는 `blockGroup` 이 비어 스키마(`blockContainer+`)를 어긴다.**
`block-selection.ts` 의 그 함수가 이미 "그룹째 처리 / 루트 그룹은 예외" 규칙을 갖고 있으니 재사용한다.
...
| 블록 이동 `Mod+Shift+↑↓` · 드래그 핸들 · 여백 드래그 선택 | 미구현(F-01-08). `Cmd+D` 복제와 `Shift+클릭`은 #27 에서 들어갔다 | `keymap.ts` · `block-selection-plugin.ts` |
| 비연속 블록 선택(`Alt+Shift+클릭`) | 없음. 정본이 P1 로 자른 것 | `block-selection.ts` |
| 브라우저에서의 타이핑 검증 | **아직 하지 않았다.** HTTP end-to-end(페이지 생성 → 본문 저장 → 왕복 → 409 충돌 → 화면 렌더)까지는 확인했고, 캐럿·IME·`/` 메뉴·**블록 선택 하이라이트**는 **헤드리스 테스트로만** 검증됐다. 특히 `visible = false` 선택의 DOM 동기화는 `EditorView` 가 필요해 확인하지 못했다(설계는 `CellSelection` 전례를 그대로 따랐다). 브라우저 자동화 도구를 아직 넣지 않았다 | — |
...
Phase 0 W1~W4 와 W5-a(페이지 트리)가 끝났고, W5-b 의 첫 항목인 멀티 블록
선택(F-01-09)도 끝났다(PR #27). 다음은 드래그·상하 이동(F-01-08)이다.
옮길 대상은 src/lib/editor/block-selection.ts 의 BlockSelection.rootPositions
가 이미 최상위 정규화된 상태로 준다 — 다시 계산하지 마라.
착수 전에 docs/research/01-block-editor.md 의 F-01-08 · F-01-12(컬럼 드롭은
P1 이다) 를 읽어라.
...
- `npm test` 만 돌리면 DB 테스트가 **조용히 빠진다**(카운트에도 안 잡힌다).
  `npm run db:up` 을 먼저 해야 545개가 다 돈다 — CI 의 db 잡과 같은 수여야 한다
