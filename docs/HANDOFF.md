...
최종 갱신: 2026-09-15 (PR #75 시점 — Phase 0 MVP · Phase 1 의 첫 항목 익스포트(F-09-14) v1 이 닫혔다. **Phase 1 CRDT 동시편집(F-05-01) 진행 중 — 1조각(Y.Doc 본문 계약 · 정규화) · 2조각(`doc_update` 로그 저장소) · 3a조각(y-prosemirror 연쇄 삭제 막기 · 수선의 작성자 한 곳) · 3b조각(멘션 · 수식 서식) · 4a조각(본문 세션 · 프로젝터 추출) · 4b조각(넘기기 — 본문의 정본이 행에서 Y.Doc 으로 넘어갔다) · **5a조각(협업 서버 골격 · 참여자 update 를 쌓은 뒤에만 퍼뜨린다)** 끝. 그 과정에서 발견한 휴지통 · 복원 · 영구 삭제 · 이동의 페이지 권한 구멍을 막았다(#75). 다음은 하위 페이지 목록 · breadcrumb 의 제목 누출(§7 맨 위) → 5b조각(정본 프로젝터 — 휴지통 전이)**, §2 CRDT 조각 표)
...
**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1591개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 208개(이 PC 에서 206 통과 — 클립보드 2개) / PR 74개 머지(#75 포함).
...
| 5b | **정본 프로젝터** — 참여자가 지운 살아 있는 하위 페이지를 거부하지 않고 휴지통으로 보낸다(§3.4 X-1 의사코드). 참여자 경로에서 그 페이지를 버릴 권한을 어떻게 볼지 · 깊이 초과를 어떻게 할지 함께 정한다 | ⬜ |
...
| 항목 | 상태 | 어디 |
|---|---|---|
| **⚠ 하위 페이지 목록 · breadcrumb 이 볼 수 없는 페이지의 제목을 내준다** | **#75 를 만들며 발견, 다음 PR.** `listChildPages`(워크스페이스 홈 · 페이지 화면 · `GET /pages?parent=`) · `listAncestors`(breadcrumb)가 권한을 거르지 않는다 — 볼 수 있는 페이지 밑의 비공개 하위 페이지 제목, 비공개 조상 밑에 따로 공유받은 페이지의 조상 제목이 보인다. `GET /pages?parent=` 는 부모를 볼 수 있는지도 보지 않는다. 사이드바 · 휴지통 · 최근 방문 · 검색 · 익스포트처럼 `readableScopes` 로 거르면 된다(§3.3-32) | `src/lib/block/page.ts` · `api/.../pages` 라우트 |
...
| **문서에서 빠진 살아 있는 하위 페이지 — 정본은 휴지통 전이, 지금은 거부** | 정본 프로젝터 의사코드는 "`type='page'` 인데 doc 에 없으면 `lifecycle='trashed'` 로 전이"다. 지금 프로젝터는 `page_ref_missing` 으로 **거부**한다(Phase 0 규칙 — 낡은 탭이 하위 페이지를 지우지 못하게). 5a 부터 참여자 update 도 거부하고 **그 문서 연결을 닫는다** — 참여자의 로컬 문서에는 지운 것이 남으므로 에디터가 붙기 전(6조각)에 정본대로 바꿔야 한다. **5b** 의 몫이고, 그 전에 휴지통 명령의 권한 규칙(이 표 맨 위)부터 정한다 | `src/lib/block/save-page-body.ts` · `body-write.ts` |
...
다음 작업은 **하위 페이지 목록 · breadcrumb 의 제목 누출**(HANDOFF §7 맨 위 — #75 를 만들며 발견)이고, 그다음이 **5b조각 — 정본 프로젝터**다.
  ① 누출: listChildPages(워크스페이스 홈 · 페이지 화면 · GET /pages?parent=) · listAncestors(breadcrumb)가 권한을 거르지 않아
     볼 수 없는 페이지의 제목이 보인다. GET /pages?parent= 는 부모를 볼 수 있는지도 보지 않는다. 사이드바 · 휴지통처럼
     perm_scope_id = ANY(readableScopes) 로 거른다(§3.3-32). 누출을 재현하는 검사를 먼저 실패시킨다
  ② 5b: 참여자가 지운 살아 있는 하위 페이지를 거부하지 않고 휴지통으로 보낸다(정본 §3.4 X-1 프로젝터 의사코드). 함께 정할 것 —
     그 페이지를 버릴 권한이 없는 참여자(휴지통 명령과 같은 규칙 — §3.2-18 의 edit_content) · page_ref_too_deep(참여자 경로에
     거부가 남으면 5d 디바운스를 할 수 없다, §3.3-98) · Phase 0 PUT 은 거부를 유지하는가(§3.3-12 낡은 탭 규칙). 5a 의 검사
     ⑦(collab-server.db.test.ts)과 body-write.db.test.ts 의 거부 검사가 그때 뒤집힌다
  ③ 그 뒤 5c(명령 · 권한 변경의 즉시 전파 · 권한 회수 시 끊기 — F-05-19) · 5d(투영 디바운스 · projected_seq) — §2 표
  ④ 착수 전에 읽을 것: 정본 §3.3 capability 매트릭스 · §3.4 상태 전이표 · X-1 프로젝터 의사코드 · src/lib/block/trash.ts · move-page.ts ·
     src/lib/collab/collab-server.ts 머리말 · block/body-write.ts 의 appendDocUpdate · HANDOFF §2 의 4b · 5a 항목 · §3.2-17 · §3.3-98
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1591개가 다 돌아야 CI 의 db 잡과 같다.
