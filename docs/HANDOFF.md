...
**동작하는 것**: 이메일 OTP 로그인 → 워크스페이스 생성 → 이메일 초대 → 수락 → 워크스페이스 진입 → 페이지 생성 → 본문 편집(12종 블록 · 분할/병합 · 중첩 · `/` 메뉴 · 마크다운 · 서식) → 자동 저장 → 하위 페이지 → 페이지 이동 → 휴지통 · 복원 · 영구 삭제 → 멀티 블록 선택 → 블록 이동 · `+` 버튼 · 토글 접기 → 블록 메뉴 · 복사/붙여넣기(중첩 보존) · 이미지(올리기 · 주소 · 드롭 · 붙여넣기) · 끊겨도 사라지지 않는 편집(협업 서버 · IndexedDB 보존본) · 페이지 공유(상속 · 따로 관리하기) · 즐겨찾기 · 최근 방문 → **코멘트 · 멘션 · 인박스**(페이지 · 글자 범위 코멘트 · 답글 · 해결 · `@` 멘션 · 백링크 · 인박스 읽음/보관 · 안 읽음 배지) → **`Cmd+K` 검색(한국어가 조사를 넘는다 · 권한 필터가 쿼리 안에)** → **풀페이지 데이터베이스**(사이드바에서 만들기 · 속성 6종 추가 · 행 추가 · 셀 편집 · 키보드 이동 · select 옵션 만들기 · "더 보기" · 필터 · 정렬 · 속성 숨기기 · 머리 메뉴) → **보드 뷰**(탭에서 만들기 · 열 = 그룹 · 카드를 끌어 값과 순서를 바꾸기 · 열 머리 `+` · 그룹 숨기기 · 그룹 기준) → **내보내기**(페이지 · 표 · 워크스페이스 전체 — Markdown & CSV ZIP · 누르면 개수부터 · 볼 수 없는 페이지는 빠지고 `_export_report.json` 이 센다).

`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 295개 항목을 확인한다**(앱과 협업 서버를 함께 띄운다). 이 PC 에서는 **Chrome 으로** 돌린다 — Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료된다(§6). 기존 클립보드 검사 2개는 이 환경에서 실패한다(§7).

**숫자**: 마이그레이션 22개 / 테이블 49개 / 테스트 1733개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 295개(이 PC 에서 293 통과 — 클립보드 2개) / PR 98개 머지(#99 포함).
...
| 4c | **status 프로퍼티(F-03-05) · List 뷰(F-04-04)** | ⬜ |
...
| **그룹 버킷이 select · checkbox 둘** | F-04-11 의 9종 중 둘. `sub_group_by` · multi_select(중복 등장) · person · date(단위) · number(구간)는 그 타입이 들어올 때 `keyExprOf` 에 규칙을 더한다. 그룹 헤더 집계(sum · average)도 없다 — 카운트만 | `group.ts` |
...
→ 풀페이지 데이터베이스(표 · 보드 · 속성 6종 · 셀 편집 · 필터 · 정렬 · 속성 숨기기 · 카드 드래그)
...
다음 작업 — 순서대로:
  ① **보드 4a · 4b 가 끝났다.** 다음은 **4c — status 프로퍼티(F-03-05) · List 뷰(F-04-04)**: status 는 `select_option.group_id` 가
     이미 있다(그룹 3개 고정 To-do · In progress · Complete · 기본 옵션 · 정본 §3.4). 보드의 그룹 키(`group.ts` `keyExprOf` —
     select 와 같은 옵션 id)와 자동 선택(`view.ts` `pickGroupProperty` — 지금은 첫 select)에 status 를 앞세운다(F-04-03 순서
     status → select). `GROUPABLE_TYPES`(property-types.ts)에 status 를 더하면 도구줄 · 보드가 따라온다. List 뷰는 **표의 축약
     렌더러**다 — 별도 컴포넌트 트리를 만들지 마라(`view-tabs.tsx` 의 CREATABLE 에 list 를 열고 `page.tsx` 가 표 컴포넌트에 모양만
     다르게 준다)
