...
최종 갱신: 2026-09-13 (PR #61 시점 — **Phase 0 MVP 가 닫혔다. Phase 1 익스포트(F-09-14) 진행 중 — 1(Markdown 직렬화기) · 2a(ZIP · CSV) · 2b(이름 · 조립 · 스트림) 끝. 다음은 3(스냅샷 읽기 · 다운로드 라우트 · 페이지 메뉴)**, §2 조각 표와 `ExportSnapshot` 계약)
...
**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1452개(CI skip 0 · 이 PC 는 unzip 이 없어 1개 skip) + 브라우저 검증 194개(이 PC 에서 192 통과 — 클립보드 2개) / PR 60개 머지(#61 포함).
...
| 3 | 스냅샷 읽기(권한 · 한 스냅샷) · 다운로드 라우트 · 페이지 메뉴 · e2e | ⬜ |
...
1(Markdown 직렬화기) · 2a(ZIP · CSV) · 2b(이름 · 조립 · 스트림 — src/lib/export/)는 끝났고
**3(스냅샷 읽기 · 다운로드 라우트 · 페이지 메뉴 · e2e)** 부터다 — HANDOFF §2 조각 표와
`ExportSnapshot` 계약, 그 아래 계획(동기 스트리밍 · REPEATABLE READ)을 먼저 읽어라.
