...
| **뷰 CRUD · 표 화면** | `view`·`view_property` 표와 질의(`queryRows`)는 있지만 **뷰를 만들고 고치는 함수와 표 UI 가 없다.** 필터·정렬 AST 는 `queryRows` 에 인자로 넘기는 형태이고, 뷰에 저장하는 경로가 다음이다 | `src/lib/database/query.ts` |
