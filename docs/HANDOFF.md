...
최종 갱신: 2026-09-10 (PR #24 머지 시점 — W5-a 진행 중)
...
| **W5-a** | **페이지 트리** | **진행 중** | #21(이동) #22(휴지통) #23(본문 안 중첩) #24(`/page`) |
| W5-b | 편집 확장 · 파일 | | |
| W6 | 내비게이션 · 권한 | | |
| W7 | 검색 | | |
| W8 | DB 코어 · 뷰 | | |
...
**숫자**: 마이그레이션 8개 / 테이블 24개 / 테스트 479개(CI skip 0) / PR 24개 머지.
...
`F-02-13`(서브페이지 — `/page`. @멘션·link_to_page 는 명세가 P1 로 자른다).
W4 에서 미리 들어간 것: `F-01-07`(Tab 중첩) · `F-01-13`(토글 접힘, 로컬 상태).

**권장 순서**
1. **사이드바 페이지 트리** (`F-02-03` / W6-a `F-07-16`) — 부분 로드(`F-01-22` API 계약)를 염두에 두고.
2. **W5-b**: **멀티 블록 선택**(`F-01-09`) → **드래그·상하 이동**(`F-01-08`) → **복붙 구조 보존**(`F-01-10`).
