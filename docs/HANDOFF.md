...
| **표 화면 (UI)** | 서버는 끝났다 — `view.ts` CRUD + API 라우트 4개(`databases` · `views/[viewId]` · `/rows` · `/columns/[propertyId]`). **화면이 없다.** `GET /rows` 가 `columns` 와 `rows` 를 한 번에 주므로 표를 그릴 데이터는 한 왕복이다 | `src/lib/database/view.ts` · `src/app/api/.../views/` |
