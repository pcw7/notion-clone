...
`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 241개 항목을 확인한다**(앱과 협업 서버를 함께 띄운다). 이 PC 에서는 **Chrome 으로** 돌린다 — Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료된다(§6). 기존 클립보드 검사 2개는 이 환경에서 실패한다(§7).

**숫자**: 마이그레이션 20개 / 테이블 45개 / 테스트 1672개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 241개(이 PC 에서 239 통과 — 클립보드 2개) / PR 93개 머지(#94 포함).
...
| 4b | **본문 글자에 붙이기** — 본문에서 고른 범위로 스레드 열기(`anchor.ts` `textRangeAnchor`) · 하이라이트 데코레이션 · 겹치는 범위. 2조각의 앵커는 있고 그것을 만드는 화면이 없다 | ⬜ |
...
| 본문 글자에 코멘트를 붙이는 화면이 없다 | 2조각의 범위 앵커(`anchor.ts`)를 만드는 곳이 아직 없다 — 화면에서 여는 스레드는 전부 페이지 스레드다. 4b 가 본문 선택 · 하이라이트를 붙인다 | `comment-panel.tsx` |
...
다음 작업 — 순서대로:
  ① 코멘트 **4b — 본문 글자에 붙이기**: 본문에서 고른 범위로 스레드를 연다. 앵커는 **편집기가 자기 Y.Doc 에서**
     만든다(`comment/anchor.ts` `textRangeAnchor` · §3.3-128) — 서버에 오프셋을 보내지 마라. 하이라이트는 데코레이션으로
     그린다(편집기 DOM 에 속성을 직접 달지 마라 — §6). 겹치는 범위의 규칙은 05 F-05-07 이 정해 뒀다
  ② 그다음 5조각(멘션). §2 의 코멘트 조각 표가 무엇까지 끝났는지 갖고 있다
  ③ 4조각이 앵커를 화면에 그릴 때: 범위는 **블록 안 오프셋**으로 온다(listDiscussions 의 anchor.range). 편집기는 자기
     Y.Doc 에서 다시 풀어 데코레이션을 그린다 — 서버가 준 오프셋을 그대로 믿으면 그 사이의 편집만큼 어긋난다.
     새 스레드의 앵커도 편집기가 자기 Y.Doc 에서 만든다(anchor.ts textRangeAnchor · §3.3-128)
