...
`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 265개 항목을 확인한다**(앱과 협업 서버를 함께 띄운다). 이 PC 에서는 **Chrome 으로** 돌린다 — Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료된다(§6). 기존 클립보드 검사 2개는 이 환경에서 실패한다(§7).

**숫자**: 마이그레이션 22개 / 테이블 47개 / 테스트 1714개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 265개(이 PC 에서 263 통과 — 클립보드 2개) / PR 97개 머지(#98 포함).
...
| 4b | **보드 화면** — 열 · 카드 · 드래그(dnd-kit 검토 — MIT) · 열 헤더 `+` · 숨김 · 뷰 탭 만들기 · 타입 전환 | ⬜ |
...
| **보드 화면이 없다** | 4a 는 서버 명령 · 라우트뿐이다(`group.ts` · `views/{id}/groups`). 4b 가 열 · 카드 · 드래그 · 뷰 탭 만들기를 만든다 | `app/w/[workspaceId]/db/` |
...
HANDOFF §2 의 **보드 조각 표**와 "보드 4a 가 정한 것"을 먼저 읽어라. 코멘트 조각 표 · CRDT 조각 표는 그쪽을 건드릴 때 읽는다.
  ...
  - 보드 4a조각(#98) db/migrations/0022_row_position.sql · src/lib/database/group.ts · view.ts(groupBy · board) · row.ts(updateCellsIn)
    ...
    databases/{id}/views · PATCH views/{id}(type · groupBy). 화면은 없다
...
다음 작업 — 순서대로:
  ① **보드 4a(저장 모양 + 서버 명령)가 끝났다.** 다음은 **4b — 보드 화면**: `app/w/[workspaceId]/db/[databaseId]/page.tsx` 가
     `view.type === 'board'` 면 표 대신 보드를 그린다(`GET views/{id}/groups` 를 서버에서 한 번 읽어 내려준다 — 표의 첫 화면과
     같은 방식). 열 = 그룹(숨긴 열은 접힌 목록에 개수만) · 카드 = 제목 + 보이는 속성 배지 · 열 하단 `+`(그 그룹 값으로 행 만들기 —
     `POST views/{id}/rows` 에 셀을 미리 채워서) · 드래그(dnd-kit: MIT — 라이선스 먼저 확인 · 놓으면 `POST groups {action:'move'}`
     하나 · 낙관적으로 옮기고 실패하면 되돌린다) · `manualOrder: false` 면 열 안 드래그 금지 · 그룹별 "더 보기"(`?group=&cursor=`) ·
     뷰 탭에 "보드 만들기"(`POST databases/{id}/views {type:'board'}` — `group_required` 면 select 를 먼저 만들라고 안내) ·
     도구줄에 그룹 속성 고르기 · 그룹 숨김 · 빈 그룹 숨김(`PATCH views/{id} {groupBy}`). e2e 로 드래그까지 확인한다
  ② 4c — status 프로퍼티(F-03-05: `select_option.group_id` 가 이미 있다 · 그룹 3개 고정 · 기본 옵션) · List 뷰(F-04-04: 표의 축약
     렌더러 — 별도 컴포넌트 트리를 만들지 마라)
