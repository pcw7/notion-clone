...
**숫자**: 마이그레이션 17개 / 테이블 39개 / 테스트 1647개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 212개(이 PC 에서 210 통과 — 클립보드 2개) / PR 83개 머지(#84 포함).
...
다음 작업 — 순서대로:
  ① 6조각(에디터 바인딩) — ySyncPlugin · origin 범위 undo(F-05-15) · 오프라인 보존(y-indexeddb) · 두 탭 e2e. 5조각이 끝나 협업 서버 ·
     로그 · 투영이 다 섰다. 지켜야 할 것이 §2 의 3a · 5a · 5c · 5d 항목과 #81(참조 제목 맵) · §7 의 "참여자의 undo 가 되살린 하위 페이지
     참조"에 쌓여 있다 — 먼저 전부 모아 읽고 조각을 자른다(바인딩 붙이기 · 거부된 연결의 로컬 문서 · undo · 오프라인 · PUT 경로 걷어내기)
  ② CRDT 다음은 마스터 문서 §5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이다
  ③ 착수 전에 읽을 것: §2 의 "6조각이 지켜야 할 것" 넷 · §3.2-14 · 15 · 17 · 20 · 22 · 25 · §7 의 협업 항목 전부 ·
     src/lib/testing/collab-peers.ts(bind) · src/lib/collab/collab-schema.ts · src/app/w/[workspaceId]/[pageId]/body-editor.tsx
