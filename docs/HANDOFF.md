...
**숫자**: 마이그레이션 18개 / 테이블 42개 / 테스트 1644개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 216개(이 PC 에서 214 통과 — 클립보드 2개) / PR 90개 머지(#91 포함).
...
| 2 | **범위 앵커**(F-05-07) — `discussion.anchor` 에 Y RelativePosition 쌍 + `quoted_text` 폴백. 앵커가 동시 편집을 견디고, 범위가 사라지면 페이지 스레드로 강등. 지금은 앵커가 블록 하나다 | ⬜ |
...
| 코멘트에 라우트 · 화면이 없다 | 1조각은 서버 명령까지다(§2 코멘트 조각 표). 부르는 곳은 검사뿐이다 | `src/lib/comment/discussion.ts` |
...
다음 작업 — 순서대로:
  ① 코멘트 **2조각 — 범위 앵커**(F-05-07): discussion.anchor 에 Y RelativePosition 쌍 + quoted_text 폴백. 앵커가 동시 편집을
     견디고, 범위가 다 지워지면 페이지 스레드로 강등한다(05 F-05-07 엣지 표). 지금 앵커는 블록 하나다
  ② 그다음 **3조각 — 알림 · 인박스**(F-11-07 · F-11-08): 정본 **§3.8**(활동 · 구독 · 알림)을 읽는다. 배달 파이프라인은 3단이고
     **순서가 고정**이다 — 대상자 산출 → **배달 시점** 권한 재검사 → 활성 뷰어 억제. payload 를 복제하지 말고 event_ids[] 로
     참조한다(N3). 병합은 저장이 아니라 조회 시점이다(N2)
  ③ 그다음 4조각(화면 · e2e) → 5조각(멘션). §2 의 코멘트 조각 표가 무엇까지 끝났는지 갖고 있다
  ④ 착수 전에 읽을 것: §2 의 코멘트 조각 표 · "1조각이 정한 것" · 정본 **§3.9**(코멘트 — 초판에 없던 보강은 2h) · **§3.8**(알림) ·
     05-collaboration-sync.md F-05-07 · F-05-08 의 엣지 케이스 표 · §3.3-125~127 · src/lib/comment/discussion.ts 머리말
