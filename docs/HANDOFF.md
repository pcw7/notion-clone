...
최종 갱신: 2026-09-13 (PR #57 시점 — **Phase 0 MVP 가 닫혔다. W8 데이터베이스는 표 화면까지 끝. 다음은 Phase 1 — 익스포트(F-09-14)**)
...
**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1320개(CI skip 0) + 브라우저 검증 194개(이 PC 에서 192 통과 — 클립보드 2개) / PR 56개 머지(#57 포함).
...
7. 커밋 → 푸시 → gh pr create     ← 작업이 끝나면 커밋 · 푸시까지 반드시 완료한다
8. CI 두 잡 통과 확인 (db 잡의 skip 이 0인지도 본다)
9. 사용자에게 머지 허락을 받고 gh pr merge --squash --delete-branch
10. 새 브랜치를 판다              ← 머지 후 main 에 서 있다
```

**작업이 끝나면 커밋 · 푸시까지 완료한다.** 사용자가 정한 규칙이다. 로컬에만 남은 커밋 · 수정은 다음 세션이
볼 수 없다 — "끝났다"고 말하기 전에 `git status` 가 깨끗하고 브랜치가 원격에 올라가 있는지 확인한다.

**머지는 사용자 허락을 받는다.** 자동 모드의 권한 분류기가 `gh pr merge` 를 "리뷰 없는 머지"로 막는다(W8-b 에서
실제로 막혔다). CI 가 초록이면 머지해도 되는지 PR 마다 묻는다.
...
작업 순서:
  ...
  → 사용자에게 머지 허락을 받고 squash 머지

**작업이 끝나면 커밋 · 푸시까지 반드시 완료한다.** 로컬에만 남은 커밋 · 수정은
다음 세션이 볼 수 없다. "끝났다"고 말하기 전에 git status 가 깨끗하고
브랜치가 원격에 올라가 있는지 확인한다.
...
- gh pr merge 는 권한 분류기가 막는다. CI 가 초록이면 사용자에게 머지 허락을 받는다
```
