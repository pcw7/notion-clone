...
최종 갱신: 2026-09-13 (PR #59 시점 — **Phase 0 MVP 가 닫혔다. Phase 1 익스포트(F-09-14) 착수 — 1조각 Markdown 직렬화기 끝. 다음은 2조각(ZIP · CSV · 파일 배치)**, §2 조각 표)
...
**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1378개(CI skip 0) + 브라우저 검증 194개(이 PC 에서 192 통과 — 클립보드 2개) / PR 58개 머지(#59 포함).
...
| 2 | ZIP 쓰기(의존성 없이 `node:zlib`) · CSV(풀페이지 DB) · 파일 배치(이름 충돌 · 상대 링크) · `_export_report.json` | ⬜ |
...
1조각(본문 → Markdown 직렬화기, src/lib/export/markdown.ts)은 끝났고 **2조각(ZIP · CSV ·
파일 배치 · 보고서)** 부터다 — HANDOFF §2 조각 표와 그 아래 계획을 먼저 읽어라.
