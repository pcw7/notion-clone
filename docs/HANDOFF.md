...
최종 갱신: 2026-09-14 (PR #66 시점 — Phase 0 MVP · Phase 1 의 첫 항목 익스포트(F-09-14) v1 이 닫혔다. **Phase 1 CRDT 동시편집(F-05-01) 진행 중 — 1조각(Y.Doc 본문 계약 · 정규화) 끝. 다음은 2조각(`doc_update` 로그 저장소)**, §2 CRDT 조각 표)
...
**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1502개(CI skip 0 · 이 PC 는 unzip 이 없어 1개 skip) + 브라우저 검증 206개(이 PC 에서 204 통과 — 클립보드 2개) / PR 65개 머지(#66 포함).
...
| 2 | `doc_update` 로그 저장소 — append(페이지 안 seq 할당) · `doc_snapshot` 압축 · 기존 페이지 옮기기(행 → Y.Doc, 한 번만) | ⬜ |
...
- `src/lib/collab/ydoc.test.ts` 의 `peer` · `edit` · `converge` — 에디터 없이 두 참여자를 흉내 내는 방법.
  `edit` 은 `ySyncPlugin` 이 쓰는 `updateYFragment` 를 부르고, client id 를 고정해 결과를 결정론으로 만든다
...
HANDOFF §2 의 CRDT 조각 표와 그 아래 근거를 먼저 읽어라. 1조각(Y.Doc 본문 계약 · 정규화)이 끝났다.
  - 다음은 2조각: doc_update 로그 저장소(append · 페이지 안 seq · doc_snapshot 압축 · 기존 페이지 옮기기)
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1502개가 다 돌아야 CI 의 db 잡과 같다.
