...
최종 갱신: 2026-09-13 (PR #53 머지 시점 — **W7 검색 완료 · W8 DB 는 서버가 끝났다. 다음은 표 화면(UI) 하나**)

---

## 1. 지금 어디까지 왔나

**Phase 0 (MVP) 중 W1–W7 완료. W8 은 서버 계층이 전부 끝났고 남은 것은 표 화면(UI) 하나다.**

> 그것을 만들면 **Phase 0 MVP 가 닫힌다.**
...
| **W8-b** | **DB 뷰** | **서버 ✅ / UI 미착수** | #52(연산자 카탈로그·컴파일러) #53(뷰 CRUD·라우트) |

**동작하는 것**: 이메일 OTP 로그인 → 워크스페이스 생성 → 이메일 초대 → 수락 → 워크스페이스 진입 → 페이지 생성 → 본문 편집(12종 블록 · 분할/병합 · 중첩 · `/` 메뉴 · 마크다운 · 서식) → 자동 저장 → 하위 페이지 → 페이지 이동 → 휴지통 · 복원 · 영구 삭제 → 멀티 블록 선택 → 블록 이동 · `+` 버튼 · 토글 접기 → 블록 메뉴 · 복사/붙여넣기(중첩 보존) · 이미지(올리기 · 주소 · 드롭 · 붙여넣기) · 끊겨도 사라지지 않는 저장(IndexedDB 큐) · 페이지 공유(상속 · 따로 관리하기) · 즐겨찾기 · 최근 방문 → **`Cmd+K` 검색(한국어가 조사를 넘는다 · 권한 필터가 쿼리 안에)**.

**서버만 있고 화면이 없는 것**: 데이터베이스. 표 · 프로퍼티 6종 · 셀 쓰기 · 필터 · 정렬 · 커서 · 뷰 CRUD 가 전부 API 로 동작한다(아래 §2).

`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 136개 항목을 확인한다**.

**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1228개(CI skip 0) + 브라우저 검증 136개 / PR 52개 머지.

---

## 2. 다음 작업 — 표 화면 (UI)

**서버는 끝났다.** 남은 것은 데이터베이스를 눌러볼 수 있게 만드는 것 하나다.

| F-ID | 무엇 | 상태 |
|---|---|---|
| `F-03-01` | DB 컨테이너(3계층) | ✅ #48 #50 |
| `F-03-02` | 프로퍼티 스키마 관리 | ✅ #50 |
| `F-03-03` `F-03-04` `F-03-06` | 프로퍼티 6종(title·rich_text·number·select·checkbox·date) | ✅ #50 — **어느 6종인지는 정본이 안 적어서 우리가 정했다**(§3.2) |
| `F-03-16` | 셀 편집 (서버) | ✅ #51 — 그리드 내비게이션은 **UI 몫** |
| `F-03-17` | 연산자 카탈로그 | ✅ #52 — **데이터로 두되 SQL 은 넣지 않았다**(§3.1) |
| `F-04-01` `F-04-12` | Table 뷰 · 표시 프로퍼티 (서버) | ✅ #53 |
| `F-04-09` `F-04-10` `F-04-15` | 필터 · 다중 정렬 · keyset 커서 | ✅ #52 |
| `F-04-14` | 풀페이지 DB | 서버 ✅ / **라우트·화면 미착수** |
...
**그 뒤**
Phase 0 이 닫힌다. Phase 1 의 최우선은 마스터 문서가 지정한 **익스포트(F-09-14)** 다
...
Phase 0 의 W1~W7 이 끝났고 W8(데이터베이스)은 **서버 계층이 전부 끝났다.**
남은 것은 표 화면(UI) 하나이고, 그것을 만들면 Phase 0 MVP 가 닫힌다.

지금 동작하는 것: 로그인 → 워크스페이스 → 페이지 → 본문 편집(12종 블록) →
자동 저장 → 휴지통 → 블록 선택·이동·메뉴·복붙·이미지 → 오프라인 저장 큐 →
페이지 공유 → 즐겨찾기·최근 방문 → Cmd+K 검색.
데이터베이스는 API 로만 동작한다 — 표·프로퍼티 6종·셀 쓰기·필터·정렬·커서·뷰 CRUD.

다음 작업은 **표 화면**이다(F-04-14 풀페이지 DB + F-03-16 의 UI 몫):
  1. /w/[workspaceId]/db/[databaseId] 라우트 + 사이드바에 DB 넣기
  2. 표 그리드 — GET /api/workspaces/{ws}/views/{viewId}/rows 가
     columns 와 rows 를 **한 왕복에** 준다
  3. 타입별 셀 렌더러·에디터 6종. 값 계약은 property-types.ts 가 갖고 있다
     (validateCellValue 로 보내기 전에 검사한다)
  4. 그리드 키보드 내비게이션 — Tab/Shift+Tab 좌우, Enter 아래, Esc 취소
  5. 필터·정렬 패널 — 연산자 드롭다운은 **filter_operator 카탈로그에서 읽는다**
     (코드에 박지 마라. 그게 그 표의 존재 이유다)
  6. "더 보기" 50행 — nextCursor 를 그대로 다시 보낸다. 무한 스크롤은 만들지 않는다
  7. scripts/e2e-editor.mjs 에 검사 추가 (화면에 기대는 것이므로 필수)

착수 전에 읽어라:
- docs/research/03-database-core.md F-03-16 (셀 편집 UX · 단축키 전수)
- docs/research/04-database-views.md F-04-01 · F-04-12 · F-04-14
- src/lib/database/view.ts 와 property-types.ts 의 머리말

표 UI 에 **TanStack 을 넣지 않기로 이미 정했다**(HANDOFF §3.2-11) —
필터·정렬·페이지네이션이 전부 서버 쪽이고 50행만 그리므로 평범한 <table> 이
코드가 더 적다. 가상화가 필요해지면 그때 @tanstack/react-virtual 을 넣는다.
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1228개가 다 돌아야 CI 의 db 잡과 같다
