...
**숫자**: 마이그레이션 20개 / 테이블 45개 / 테스트 1676개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 256개(이 PC 에서 254 통과 — 클립보드 2개) / PR 94개 머지(#95 포함).
...
| 5 | **멘션(F-07-08 · F-05-09)** — `@` 자동완성 · 본문의 `mention` 노드와 역인덱스 · 멘션 알림 | ⬜ |
...
다음 작업 — 순서대로:
  ① 코멘트 **5조각 — 멘션**(F-07-08 · F-05-09): `@` 자동완성 · 본문의 `mention` 노드와 **역인덱스** · 멘션 알림.
     마스터 문서 §5.2: *"멘션은 정규식 파싱이 아니라 **에디터가 `mention` 인라인 노드를 만들 때 역인덱스를 함께
     기록**한다"*. 본문의 한 자리를 가리키는 값은 **Y.Doc 을 가진 쪽이 만든다**(§3.3-128 · 139) — 역인덱스도 같은 함정이
     있다. 알림 종류 `mention` 은 이미 CHECK 에 있고 팬아웃만 붙이면 된다(`notification/fanout.ts`)
