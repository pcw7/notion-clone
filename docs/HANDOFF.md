...
최종 갱신: 2026-09-10 (PR #23 머지 시점 — W5-a 진행 중)
...
| **W5-a** | **페이지 트리** | **진행 중** | #21(이동) #22(휴지통) #23(본문 안 중첩) |
| W5-b | 편집 확장 · 파일 | | |
| W6 | 내비게이션 · 권한 | | |
| W7 | 검색 | | |
| W8 | DB 코어 · 뷰 | | |
...
**숫자**: 마이그레이션 8개 / 테이블 24개 / 테스트 471개(CI skip 0) / PR 23개 머지.
...
**권장 순서**
1. **서브페이지 인라인 생성** (`F-02-13`) — `/` 메뉴에 "페이지"를 넣어 본문에서 하위 페이지를 만든다.
   `slash-menu.ts` 의 카탈로그는 `Record<MvpBlockType, …>` 이라 `page` 를 넣으려면 타입을 손봐야 한다.
   본문 안 중첩은 이미 된다(PR #23) — 만들어서 토글 안에 넣는 것까지 동작한다.
2. **사이드바 페이지 트리** (`F-02-03` / W6-a `F-07-16`) — 부분 로드(`F-01-22` API 계약)를 염두에 두고.
3. **W5-b**: **멀티 블록 선택**(`F-01-09`) → **드래그·상하 이동**(`F-01-08`) → **복붙 구조 보존**(`F-01-10`).
