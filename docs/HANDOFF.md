...
최종 갱신: 2026-09-13 (PR #60 시점 — **Phase 0 MVP 가 닫혔다. Phase 1 익스포트(F-09-14) 진행 중 — 1(Markdown 직렬화기) · 2a(ZIP · CSV) 끝. 다음은 2b(파일 배치 · 조립 · 보고서)**, §2 조각 표)
...
**동작하는 것**: 이메일 OTP 로그인 → 워크스페이스 생성 → 이메일 초대 → 수락 → 워크스페이스 진입 → 페이지 생성 → 본문 편집(12종 블록 · 분할/병합 · 중첩 · `/` 메뉴 · 마크다운 · 서식) → 자동 저장 → 하위 페이지 → 페이지 이동 → 휴지통 · 복원 · 영구 삭제 → 멀티 블록 선택 → 블록 이동 · `+` 버튼 · 토글 접기 → 블록 메뉴 · 복사/붙여넣기(중첩 보존) · 이미지(올리기 · 주소 · 드롭 · 붙여넣기) · 끊겨도 사라지지 않는 저장(IndexedDB 큐) · 페이지 공유(상속 · 따로 관리하기) · 즐겨찾기 · 최근 방문 → **`Cmd+K` 검색(한국어가 조사를 넘는다 · 권한 필터가 쿼리 안에)** → **풀페이지 데이터베이스**(사이드바에서 만들기 · 속성 6종 추가 · 행 추가 · 셀 편집 · 키보드 이동 · select 옵션 만들기 · "더 보기" · 필터 · 정렬 · 속성 숨기기 · 머리 메뉴).

`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 194개 항목을 확인한다.** 이 PC 에서는 **Chrome 으로** 돌린다 — Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료된다(§6). 기존 클립보드 검사 2개는 이 환경에서 실패한다(§7).

**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1413개(CI skip 0 · 이 PC 는 unzip 이 없어 1개 skip) + 브라우저 검증 194개(이 PC 에서 192 통과 — 클립보드 2개) / PR 59개 머지(#60 포함).

---

## 2. 다음 작업 — Phase 1

**Phase 0 이 닫혔다.** Phase 1 의 최우선은 마스터 문서가 지정한 **익스포트(F-09-14)** 다
("데이터 인질 상태는 신뢰 비용"). 그다음이 CRDT 동시편집(F-05-01) · 공개 링크(F-06-06) ·
인라인 DB 다. 착수 전에 마스터 문서 §5 의 Phase 1 과 해당 도메인 문서의 F-ID 절을 읽는다.
...
| 2b | 파일 배치(이름 소독 · 충돌 · 상대 링크) · 조립(스냅샷 → 항목) · `_export_report.json` · ZIP 스트림 | ⬜ |
| 3 | 서브트리 읽기(권한 · 한 스냅샷) · 다운로드 라우트 · 페이지 메뉴 · e2e | ⬜ |

계획(아직 판결 아님): 정본에 `export_job` 표가 **없고** 잡 워커도 없다(마스터 문서: "스케줄러는 하나만").
그래서 v1 은 **동기 스트리밍 다운로드**로 시작하고 상한을 넘으면 거부한다(자르지 않는다 — 구멍 난 백업).
⚠ `withReadTransaction` 은 `BEGIN READ ONLY` 라 **READ COMMITTED** 다 — 문장마다 스냅샷이 달라진다.
서브트리 전체를 읽는 익스포트는 `REPEATABLE READ` 가 필요하다(머리말의 "스냅샷 일관성" 주장과도 어긋난다, §7).

2b 가 부딪힐 것 (2a 에서 실측 · 결정):
- **이모지(BMP 밖 글자)가 든 파일 이름은 bsdtar(이 PC 의 `tar.exe`, libarchive 3.8.8)가 풀지 못한다** — 이름을 하나씩
  ...
  노션 제목에는 이모지가 흔하다 → 파일 **이름**에서만 BMP 밖 글자를 바꾸고 문서 안 `# 제목` 은 그대로 두는 쪽을 검토한다
...
| 63 | **ZIP · CSV 는 우리가 짜지 않은 구현으로 읽어 본다 — CI 에서는 python 이 필수다** (#60) | §3.3-60 과 같은 이유. python `zipfile`(CRC · 이름 · UTF-8 플래그 · 내용) · `csv`(따옴표 규칙), bsdtar(풀기), unzip(`-t`). `src/lib/testing/external-tools.ts` 가 찾고, **CI 에서 python 이 없으면 실패**한다(`REQUIRE_DB` 와 같은 이유). 반사실: UTF-8 플래그를 끄면 python · bsdtar 검사가 실패 |
...
⚠ 자동 모드의 권한 분류기가 `gh pr merge` 를 "리뷰 없는 머지"로 막은 적이 있다(W8-b). 막히면 **우회하지 말고
...
| **e2e 환경 · 흔들림** | 이 PC 의 헤드리스 브라우저에서 클립보드 검사 2개가 실패한다(§6). 검색 오버레이의 "입력이 본문에 새지 않았다"가 7번 중 2번 실패했다 — 원인 모름, 실패하면 달라진 글자를 남기게 했다. "앞 절 이미지가 비동기로 '불러올 수 없습니다'를 그린다"는 **확인하지 않은** 후보다 | `scripts/e2e-editor.mjs` |
...
| **`withReadTransaction` 은 스냅샷을 보장하지 않는다** | 머리말은 *"스냅샷 일관성이 필요한 조회에 쓴다"* 인데 `BEGIN READ ONLY` 는 **READ COMMITTED** 라 문장마다 새 스냅샷이다. `readableScopes` 로 스코프를 읽고 다음 문장에서 행을 읽는 사이에 커밋된 권한 변경이 보인다. 읽기 전용 트랜잭션은 `REPEATABLE READ` 에서 직렬화 실패가 나지 않으므로 바꾸는 비용은 작다 — 두 문장 사이에 끼어든 커밋이 보이는지로 **먼저 재현하고** 바꾼다. 익스포트 3조각(서브트리 전체 읽기)이 부딪힌다 | `src/lib/db/tx.ts` |
...
다음 작업은 **Phase 1 — 익스포트(F-09-14)** 다(마스터 문서: "데이터 인질 상태는 신뢰 비용").
1조각(Markdown 직렬화기) · 2a(ZIP · CSV — src/lib/export/)는 끝났고 **2b(파일 배치 · 조립 ·
보고서)** 부터다 — HANDOFF §2 조각 표, 그 아래 계획과 "2b 가 부딪힐 것"을 먼저 읽어라.
그다음이 CRDT 동시편집(F-05-01) · 공개 링크(F-06-06) · 인라인 DB 다.
착수 전에 마스터 문서 §5 의 Phase 1 과 해당 도메인 문서의 F-ID 절을 읽어라.
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1320개가 다 돌아야 CI 의 db 잡과 같다
...
- gh pr merge 를 권한 분류기가 막으면 우회하지 말고 사용자에게 알린다 —
  머지는 이미 허락된 작업이니 권한 규칙을 추가해 달라고 부탁한다
```
