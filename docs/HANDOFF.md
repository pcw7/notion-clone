...
최종 갱신: 2026-09-10 (PR #25 머지 시점 — **W5-a 완료**)

---

## 1. 지금 어디까지 왔나

**Phase 0 (MVP, 6~8주) 중 W1–W4 · W5-a 완료. 다음은 W5-b.**
...
**숫자**: 마이그레이션 8개 / 테이블 24개 / 테스트 498개(CI skip 0) / PR 25개 머지.
...
1. **멀티 블록 선택** (`F-01-09`) — `Escape` 로 진입하는 블록 선택 모드는
   `keymap.ts` 의 `selectBlockCommand()` 로 자리만 잡아 뒀다. `Shift+↑↓` 로 범위 확장부터.
2. **드래그·상하 이동** (`F-01-08`) — 블록 핸들. 사이드바 드래그(`F-02-03`)도 같은 조작이지만
   그건 **권한 변경 트랜잭션**이라(F-07-16: "드래그로 섹션을 넘으면 권한이 바뀐다") W6 이후다.
