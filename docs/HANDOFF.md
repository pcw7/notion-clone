...
최종 갱신: 2026-09-15 (PR #79 시점 — Phase 0 MVP · Phase 1 의 첫 항목 익스포트(F-09-14) v1 이 닫혔다. **Phase 1 CRDT 동시편집(F-05-01) 진행 중 — 1조각(Y.Doc 본문 계약 · 정규화) · 2조각(`doc_update` 로그 저장소) · 3a조각(y-prosemirror 연쇄 삭제 막기 · 수선의 작성자 한 곳) · 3b조각(멘션 · 수식 서식) · 4a조각(본문 세션 · 프로젝터 추출) · 4b조각(넘기기 — 본문의 정본이 행에서 Y.Doc 으로 넘어갔다) · 5a조각(협업 서버 골격 · 참여자 update 를 쌓은 뒤에만 퍼뜨린다) · 5b조각(정본 프로젝터 — 참여자가 지운 하위 페이지를 휴지통으로) · **5c조각(커밋 신호 — 명령이 쓴 것을 곧바로 퍼뜨리고 권한이 줄면 서버가 먼저 닫는다)** 끝. 5a 에서 발견한 휴지통 · 이동의 페이지 권한 구멍(#75)과 목록 · breadcrumb 의 제목 누출(#76)도 막았다. **다음은 5c 에서 발견한 이동의 스코프 덮어쓰기 누출(§7 맨 위)** → 부모 본문 참조의 제목 설계 → 5d**, §2 CRDT 조각 표)
...
**숫자**: 마이그레이션 16개 / 테이블 39개 / 테스트 1603개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 208개(이 PC 에서 206 통과 — 클립보드 2개) / PR 78개 머지(#79 포함).
...
- ⚠ **발견했지만 고치지 않은 것 — 자기 ACL 을 가진 페이지를 옮기면 `perm_scope_id` 가 대상의 스코프로 덮인다**(§7 맨 위). 권한 표를
...
다음 작업 — 순서대로:
  ① **이동이 스코프 경계 페이지의 perm_scope_id 를 덮어쓰는 권한 누출을 고친다**(§7 맨 위 — #79 에서 진단으로 확인했다). 5d 보다
     먼저다. relocateSubtree 는 옮긴 페이지가 경계(자기 ACL 이 있거나 상속을 끊음)면 자기 스코프를 지키고, 옛 스코프를 따르던
     자손만 새 스코프로 옮겨야 한다. move-page.db.test.ts 의 "루트 아래로 들어가면 서브트리 전체가 새 스코프를 받는다"가 틀린 동작을
     단언한다 — 먼저 실패하는 검사(상속을 끊은 비공개 페이지를 공개 페이지 밑으로 → 멤버의 하위 목록 · 사이드바 · 검색에 제목이
     없어야 한다)로 재현한다. 복원(B4) · 프로젝터의 자리 옮기기도 같은 함수를 쓴다
  ② 부모 본문의 하위 페이지 참조가 볼 수 없는 페이지의 제목을 싣는다(§7) — 참조 노드에서 제목을 빼는 설계가 먼저다. 에디터
     바인딩(6조각) 전에 정하고 다른 작업과 한 PR 에 섞지 마라
  ③ 5d(투영 디바운스 · projected_seq) — 그 전에 참여자 경로의 page_ref_too_deep 거부를 풀어야 한다(§7 · §3.3-98)
  ④ 착수 전에 읽을 것: 정본 §3.11 perm_scope 재계산 트리거 · src/lib/block/move-page.ts 머리말 · src/lib/permissions/acl.ts 머리말 ·
     HANDOFF §2 의 5c 항목 · §3.2-18 · §3.2-20
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1603개가 다 돌아야 CI 의 db 잡과 같다.
