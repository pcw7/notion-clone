...
최종 갱신: 2026-09-16 (PR #82 시점 — Phase 0 MVP · Phase 1 의 첫 항목 익스포트(F-09-14) v1 이 닫혔다. **Phase 1 CRDT 동시편집(F-05-01) 진행 중 — 1조각(Y.Doc 본문 계약 · 정규화) · 2조각(`doc_update` 로그 저장소) · 3a조각(y-prosemirror 연쇄 삭제 막기 · 수선의 작성자 한 곳) · 3b조각(멘션 · 수식 서식) · 4a조각(본문 세션 · 프로젝터 추출) · 4b조각(넘기기 — 본문의 정본이 행에서 Y.Doc 으로 넘어갔다) · 5a조각(협업 서버 골격 · 참여자 update 를 쌓은 뒤에만 퍼뜨린다) · 5b조각(정본 프로젝터 — 참여자가 지운 하위 페이지를 휴지통으로) · **5c조각(커밋 신호 — 명령이 쓴 것을 곧바로 퍼뜨리고 권한이 줄면 서버가 먼저 닫는다)** 끝. 5a 에서 발견한 휴지통 · 이동의 페이지 권한 구멍(#75)과 목록 · breadcrumb 의 제목 누출(#76)도 막았다. 5c 에서 발견한 이동 · 복원의 권한 스코프 구멍(#80 — 볼 수 없는 페이지의 제목 누출 · 최상위로 꺼낸 페이지를 아무도 못 봄)과 부모 본문의 하위 페이지 참조가 싣던 제목(#81)도 막았다. 5d 의 선결 조건이던 참여자 경로의 깊이 초과 거부도 풀었다(#82 — 들어갈 수 있는 깊이까지 올린다). 다음은 본문 참조가 만드는 유령 페이지 막기 → 5d**, §2 CRDT 조각 표)
...
`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 212개 항목을 확인한다.** 이 PC 에서는 **Chrome 으로** 돌린다 — Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료된다(§6). 기존 클립보드 검사 2개는 이 환경에서 실패한다(§7).

**숫자**: 마이그레이션 16개 / 테이블 39개 / 테스트 1623개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 212개(이 PC 에서 210 통과 — 클립보드 2개) / PR 81개 머지(#82 포함).
...
| 5c | **즉시 전파 · 권한 회수 시 끊기(F-05-19)** — 쓰는 트랜잭션의 트리거가 NOTIFY 로 커밋 신호를 보내고(마이그레이션 0016) 협업 서버가 LISTEN 한다(`collab/change-feed.ts`). **퍼뜨리기는 신호를 커밋 순서대로 따라 한 줄로** — 권한 신호를 받으면 그 워크스페이스 연결을 다시 판정해 닫은 뒤에야 다음 꼬리를, 신호가 알린 seq 까지만 적용한다. 강등은 읽기 전용 전환이 아니라 `forbidden` 으로 닫는다. 첫 동기화 전에 한 번 더 판정 · 끊겼다 붙으면 전부 다시 맞춘다 | ✅ #79 |
| 5d 전 | **참여자 경로의 깊이 초과 거부 풀기**(§3.2-23) — 하위 페이지 참조를 그 서브트리가 깊이 상한을 넘는 자리로 옮긴 update 를 거부하지 않고 **들어갈 수 있는 깊이까지 올린다**(`normalize.ts` `page_ref_lifted` · `body-write.ts` `pageRefDepthLimits`). 투영과 수선이 같은 받은 깊이로 읽는다. 함께 고친 것: 더 깊어지지 않는 이동은 거부하지 않는다(`relocateSubtree`) · 참조를 담은 블록을 옮기면 하위 페이지 경로가 따라간다 | ✅ #82 |
...
- ~~깊이 초과(`page_ref_too_deep`)는 참여자 경로에서도 아직 거부다 — 5d(디바운스) 전에 풀어야 한다(§3.3-98)~~ → **#82 에서 풀었다**(§3.2-23)

5c조각에서 한 것 · 확인한 것 (§3.2-20):
- **Postgres NOTIFY 를 진단 스크립트로 먼저 쟀다(PG 16.15)** — 커밋 뒤에만 간다 · 롤백한 것도 **되돌린 savepoint 안의 것도** 버려진다 ·
  페이로드는 8000 바이트 미만(7999 통과 · 8000 거부) · 한 트랜잭션 안의 같은 (채널, 페이로드)는 하나로 합쳐지고 채널이 달라도 보낸
  순서가 유지된다 · 파티션 표(`doc_update`)의 행 트리거가 돈다 · **듣는 연결이 끊긴 사이의 신호는 사라지고 다시 LISTEN 해도 오지
  않는다** · 끊긴 pg 클라이언트는 다시 쓸 수 없다(`error` 두 번 뒤 `end`)
- **신호는 쓰는 트랜잭션의 트리거가 보낸다**(마이그레이션 0016) — `collab_doc`(`{page}:{seq}`) · `collab_access`(`ws:{id}` · `user:{id}`).
  권한 판정이 읽는 표 전부(ACL · 상속 메타 · 페이지의 lifecycle/스코프/경로 · 멤버 · SSO · 세션)에 걸었다. 앱에서 보내면 한 곳만 빠져도
  회수된 연결이 조용히 계속 받는다
- **퍼뜨리기는 신호를 따라 한 곳에서** — 참여자 update 도 쌓은 뒤 자기 seq 의 신호가 적용되기를 기다린다. 권한 신호를 받으면 연결을
  다시 판정해 닫은 **뒤에** 다음 꼬리를, **신호가 알린 seq 까지만** 적용한다(§3.3-101 · 102). 끊겼다 붙으면 꼬리를 먼저 읽고 → 전부 다시
  판정하고 → 먼저 읽은 꼬리를 적용한다
- **Hocuspocus 4.7.0 소스로 확인한 것** — 읽기 전용 연결이 보낸 update 는 닫지 않고 SyncStatus false 로 **조용히 버린다**(그래서 강등은
  읽기 전용 전환이 아니라 `forbidden` 으로 닫는다) · `onAuthenticate` 와 연결 등록 사이에 문서 불러오기가 끼어 있다(그래서 첫
  동기화 SyncStep1 에 답하기 전에 한 번 더 판정한다, §3.3-103) · `afterLoadDocument` 는 문서를 `documents` 맵에 넣기 전에 불린다
- 반사실이 통과해 주석을 고친 것 둘 — 신호 버퍼 · 불러온 뒤 꼬리 읽기(§3.3-104)
- ~~발견했지만 고치지 않은 것 — 자기 ACL 을 가진 페이지를 옮기면 `perm_scope_id` 가 대상의 스코프로 덮인다~~ → **#80 에서 고쳤다**(§3.2-21). 권한 표를
  훑다가 `relocateSubtree` 의 "경계는 최상위뿐" 주석을 보고 진단으로 쟀다 — 상속을 끊고 소유자만 남긴 페이지를 공개 페이지 밑으로
  옮기면 멤버는 볼 수 없는데 하위 목록 · 사이드바가 제목을 내준다. 5c 와 경계가 달라 섞지 않았다

6조각(에디터 바인딩)이 지켜야 할 것 — 5c 가 정한 것:
- 닫힘 이유로 갈라 다시 연다 — `forbidden`(강등): 로컬 Y.Doc 을 버리고 **읽기 전용으로** 다시 연다 · `not_found`: 페이지를 닫고 "접근
  권한 없음"(없는 페이지와 가르지 않는다) · `unauthenticated`: 로그인으로 · `unavailable`: 잠시 뒤 다시 연다. 버리는 편집은 알린다
- 에디터 · 협업 서버 어디에도 메모리 문서에 update 를 직접 적용하는 길을 새로 만들지 않는다 — 퍼뜨리기는 신호를 따라 한 곳이다
- 권한 판정이 새 표를 읽게 되면(teamspace · group · 공개 링크) **0016 과 같은 트리거를 그 표에 건다** — 빠지면 그 쓰기로 회수된 연결이 남는다
- (#81) **참조 노드는 제목을 싣지 않는다.** 협업 편집기의 노드 뷰도 권한으로 거른 제목 맵(`pageRefTitle` deps)에서 읽는다 — Y.Doc 에서
  제목을 찾지 마라. 맵을 어디서 받을지(본문 GET 한 번 · 따로 라우트)와 참여자가 새 참조를 받았을 때 맵을 채우는 길을 정한다

깊이 초과를 푼 것(#82)에서 한 것 · 확인한 것 (§3.2-23):
- **진단으로 먼저 쟀다** — ① 참여자가 참조를 깊이 100 자리로 옮기면 `page_ref_too_deep` 로 거부했다(협업 서버는 연결을 닫는다) ② 본문은
  페이지에서 센 깊이만 막아 **상한을 넘은 서브트리가 정상 편집으로 생긴다**(깊이 1 페이지 아래 경로 101) — 그 페이지는 같은 깊이 · 더 얕은
  곳으로도 옮길 수 없었다 ③ **참조를 담은 토글을 다른 블록 밑으로 옮기면 토글 행만 따라가고 하위 페이지 서브트리의 경로는 옛 자리에
  남았다** — 투영이 참조의 부모만 비교했고 깊이 검사도 건너뛰었다. ②③ 을 먼저 고쳐야 올리기가 늘 끝나고(최상위까지 가면 반드시 들어간다)
  토글째 옮기는 경우까지 잡힌다
- **§7 의 후보 둘 대신 올리기** — 되돌리기는 옛 자리가 Y.Doc 에 없고(행의 `order_key` 뿐) 같은 update 가 그 자리를 지웠을 수 있다. 행 자리
  유지는 "행 = Y.Doc 투영"(4b)을 깬다. 올리기는 문서와 서브트리 높이만으로 정해지고 정규화의 "깊이 상한을 넘는 자식은 올린다"와 같은 모양이다
- **투영과 수선이 같은 받은 깊이로 읽는다** — `readBodyYDoc` · `repairBodyYDoc` · 본문 세션의 `read` · `commit` 이 `pageRefDepth` 를 받는다. 올린
  Y.Doc 은 받은 깊이 없이 읽어도 같은 문서라 다른 읽는 곳(협업 참여자 · `loadDocState` · 검사)은 그대로다
- **HANDOFF 가 짚은 동시성 한계를 쟀다** — 올리기와 동시에 담은 토글 · 형제 · 이웃 블록에 친 글자는 남는다(`repair.test.ts` ⑤). 옮기는 것은 글자
  없는 참조 요소뿐이라 §7 "수선과 동시에 옮긴 블록에 친 글자"에 걸리지 않는다
- ⚠ **발견했지만 고치지 않은 것 — 유령 페이지**(§7). 같은 참조를 둘이 동시에 다른 곳으로 옮기면(두 참여자 · 올리기와 참여자 하나) 참조가 둘이
  되고 정규화가 둘째에 파생 id 를 준다 — 투영은 그 id 로 **제목 없는 살아 있는 `page` 행을 만든다.** 본문 저장(PUT)에 모르는 id 의 참조를 넣어도
  같고, 다른 곳의 페이지를 참조하면 PK 위반으로 던진다(500). 깊이와 경계가 달라 섞지 않았다

5d(투영 디바운스)가 지켜야 할 것 — #82 가 정한 것:
- 올리기의 받은 깊이는 **투영하는 순간의 행**에서 잰다(`pageRefDepthLimits`). 투영을 쌓기 뒤로 미루면 쌓을 때와 투영할 때의 행이 다를 수 있다
  — 올리기는 **투영하는 단위가 그 행으로 재고 수선으로 쌓아야** 한다. 수선을 쓰는 곳이 둘로 갈라지지 않게 같은 줄(스냅샷 잠금)에 선다
- 참여자 경로에 남은 거부는 둘이다 — `page_ref_forbidden`(참조를 지운 하위 페이지를 버릴 권한) · 본문에 모르는 노드가 있을 때의
  `page_ref_too_deep`(스키마 버전 게이트가 오면 닿지 않는다). 투영을 떼기 전에 둘을 쌓기 때 볼지 정한다
...
5. **e2e 환경** — 이 PC 의 Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료되고, Chrome 은 그 붙여넣기가 붙지 않는다.
   `E2E_BROWSER` 로 Chrome 을 쓰고 클립보드 검사 2개는 실패로 남는다. 검색 오버레이의 누출 검사가
   간헐적으로 떨어지던 원인은 이미지 노드 뷰의 비동기 오류 문구였다 — 편집 가능한 글자만 비교하게 고쳤다(#81 · §3.3-108)
...
| 20 | **다른 프로세스가 쓴 것 · 권한 변경을 협업 서버가 어떻게 알고 무엇을 하는가** (정본 §3.7 은 "권한 회수 시 서버 강제 unsubscribe"까지만, 05 F-05-19 는 클론 대안 (c) "권한 변경 시 space 의 모든 연결에 일괄 무효화"까지만 적었다. 5a 는 명령이 쓴 것을 다음 참여자 편집 때까지 퍼뜨리지 못했다) | **쓰는 트랜잭션의 트리거가 Postgres NOTIFY 로 커밋 신호를 보내고**(마이그레이션 0016 · `collab_doc` `{page}:{seq}` · `collab_access` `ws:{id}` · `user:{id}`) **협업 서버가 LISTEN 한다**(`collab/change-feed.ts`). **메모리 문서에 적용하는 곳은 신호를 따르는 꼬리 적용 하나**다 — 참여자 update 도 쌓은 뒤 자기 seq 의 신호를 기다린다. 권한 신호를 받으면 그 워크스페이스(세션이면 그 사용자)의 연결을 **연결을 받을 때와 같은 판정**으로 다시 보고 닫은 **뒤에야** 다음 꼬리를, **신호가 알린 seq 까지만** 적용한다. 강등(쓰기 → 보기)은 읽기 전용 전환이 아니라 `forbidden` 으로 **닫는다**. 첫 동기화(SyncStep1)에 답하기 전에 한 번 더 판정한다. 끊겼다 붙으면 꼬리 먼저 읽기 → 전부 다시 판정 → 적용. 판정하지 못하면 `unavailable` 로 닫는다 (#79) | ① **트리거인 이유**: 권한 판정이 읽는 것을 바꾸는 쓰기가 `acl.ts` 넷 · 이동 · 휴지통 · 복원 · 멤버 · 세션 · SSO 에 흩어져 있고 SQL 로 고치는 경로(검사 ④ 도 세션을 SQL 로 폐기한다)까지 있다 — 앱에서 보내면 한 곳만 빠져도 회수된 연결이 조용히 남는다. 0012 가 권한 축을 트리거로 따라가는 것과 같다 ② **NOTIFY 인 이유**: 커밋될 때만 · 커밋 순서로 간다(진단). Valkey pub/sub 는 커밋 뒤에 따로 보내야 해서 커밋과 보내기 사이에 죽으면 잃고, 트랜잭션과 순서를 맞추지 않는다. 대가: 듣는 연결이 끊긴 사이의 신호를 잃는다 → 다시 붙으면 전부 다시 맞춘다 ③ **신호 하나로 퍼뜨리는 이유**(§3.3-101 · 102): 길이 둘이면 서로 앞질러 회수 뒤에 커밋된 본문이 회수된 연결에 간다 ④ **워크스페이스 단위인 이유**: 영향받는 서브트리 × 연결을 신호마다 계산하지 않는다(F-05-19 "O(서브트리 × 구독자)"). 받는 쪽이 같은 판정 함수를 부르므로 정확성은 같고 비용만 다르다 — 정밀 무효화는 05 문서가 v2 로 미뤘다(§7) ⑤ **강등을 닫는 이유**: Hocuspocus 의 읽기 전용 연결은 보낸 update 를 닫지 않고 버리고 provider 는 자기가 읽기 전용이 된 것을 모른다(4.7.0 소스) — F-05-19 "조용한 유실 금지". 올라간 권한은 닫지 않는다(§7) ⑥ **SyncStep1 판정**: `onAuthenticate` 와 연결 등록 사이에 문서 불러오기가 끼어, 그 사이 회수는 다시 판정할 연결이 아직 없다(§3.3-103) ⑦ **판정 실패를 닫는 이유**: 보안 경계에서는 회수를 모른 채 두는 쪽이 더 나쁘다 — 검사는 없다(§7). 반사실(코드): 본문 신호를 무시하면 ⑧ ⑩ · 쌓자마자 적용하면 ⑩ 의 전제 · 판정을 안 기다리면 ⑩ "회수 뒤 명령" · seq 를 넘어 읽으면 ⑩ "seq 를 넘어" · SyncStep1 판정을 빼면 ⑪(검사를 고친 뒤 — §3.3-103) · 강등을 안 닫으면 ⑨ · 다시 붙은 뒤 판정을 빼거나 적용과 순서를 뒤집으면 ⑫ · `onResync` 를 안 부르면 신호 ③ · ⑫ · 권한 신호를 무시하면 ⑨ ⑩ ⑪ — 각각 그 항목만 실패. 신호 버퍼 · 불러온 뒤 꼬리 읽기는 반사실이 통과했다(§3.3-104). 반사실(트리거 8개를 하나씩 끔): `acl_entry` · `user_session` · `workspace_member` · `block` 은 커밋 신호 ① 과 ⑨ 의 그 연결 단언("demoted" · "loggedOut" · "removed" · "trashed"), `block_acl_meta` · `sso_config` · 세션 삭제는 커밋 신호 ①, `doc_update` 는 신호 ① · ⑧ — 처음에는 둘이 가려내지 못해 검사를 고쳤다(§3.3-106) |
| 21 | **이동 · 복원이 권한 스코프와 최상위 권한을 어떻게 정하는가** (정본 §3.11 은 스코프의 정의와 "서브트리 이동이 재계산 트리거"까지만, 06 F-06-20 은 "이동 즉시 새 부모의 권한이 상속 · 명시 ACL 은 유지", 정본 B4 는 "복원 실행자의 Private 루트로"까지만 적었다. MVP 에는 teamspace · Private 루트가 없다) | **경계(자기 ACL · 상속 끊기)인 페이지는 옮겨도 자기 스코프를 지키고, 경계가 아닐 때만 새 부모의 스코프를 받는다** — 자손은 옮긴 페이지의 옛 스코프를 따르던 것만 따라간다. **최상위로 오면 상속 원천을 ACL 행으로 둔다**: 옮기기는 워크스페이스에서 상속(`workspace_everyone → full_access` — 최상위에 만들 때와 같은 `inheritFromWorkspace`. 상속을 끊은 페이지는 받지 않고, 이미 있는 행은 `full_access` 로 올린다), 부모가 사라져 되살리기(B4)는 **되살린 사람만**(`grantToRestorer` — 그 사람에게 `full_access`). 최상위에서 다른 페이지 밑으로 갈 때 `workspace_everyone` 행은 지우지 않는다 (#80) | ① **정의를 따랐다**: 스코프 = 자기 또는 가장 가까운 경계 조상. 목록 · 사이드바 · 검색은 스코프로 거르고 `effective()` 는 조상 사슬로 판정하므로, 스코프가 정의와 어긋나면 두 길이 다르게 답한다 — 진단에서 볼 수 없는 페이지의 제목이 나갔고 볼 수 있는 페이지가 사이드바에서 사라졌다 ② **최상위에 행을 두는 이유**: `effective()` 는 허용의 합집합뿐이라 ACL 이 없는 최상위 노드는 아무도 못 본다 — 진단에서 최상위로 꺼낸 페이지 · B4 로 되살린 페이지를 옮기거나 되살린 사람조차 못 봤고 되돌릴 길도 없었다. 0010 이 "워크스페이스 멤버면 본다"를 루트 행으로 옮겼으니 최상위의 상속 원천은 그 행이다 ③ **이동과 복원을 가른 이유**: 이동은 사람이 최상위를 고른 것이다(F-06-20 "새 부모의 권한이 상속"). 복원은 자리가 사라져 우리가 올린 것이라, 워크스페이스에서 상속시키면 비공개 부모 밑에 있던 페이지가 복원 한 번으로 모두에게 열린다 — 정본 B4 의 Private 루트를 따랐다. 되살리려면 이미 `edit_content` 가 있었다(§3.2-18) ④ **지우지 않은 것**: 최상위 페이지를 비공개 페이지 밑으로 옮겨도 `workspace_everyone` 행이 남아 모두에게 열려 있다 — 명시 부여와 가를 출처 열이 없다(§7). B4 의 명시 부여도 지우지 않는다(정본 `move_to_private` 은 지운다 — 되돌릴 수 없는 쪽이라 남는 쪽으로) ⑤ **최상위 행을 한 함수로**: `createPage` · `createDatabase` 가 각자 넣던 행을 `inheritFromWorkspace` 로 모았다 — 옮길 때 그 한 벌이 빠져 있던 것이 이번 구멍이다. 먼저 실패하는 검사 4개로 재현했다. 반사실: 늘 대상의 스코프(고치기 전 규칙)면 경계 지킴 · 누출 · 토글 안으로 3개 · 최상위 행을 안 넣으면 2개 · B4 도 워크스페이스에서 상속하면 1개 · 끊은 노드에도 넣으면 1개 · 기존 행을 안 올리면 1개 — 각각 그 주장의 검사만 실패 |
| 22 | **하위 페이지 참조의 제목을 어디에 두고 누가 보는가** (정본은 참조 노드의 속성을 정하지 않았다 — X-1 · X-3 은 "참조 노드 삽입 · 제거"까지만 적었다. #76 이 목록 · breadcrumb 을 막은 뒤 본문의 참조가 남았다) | **참조 노드는 제목을 싣지 않는다** — schema 의 `page_ref` 에 `title` attr 이 없다. 제목은 본문을 읽을 때 **권한으로 거른 맵**(`loadPageBody` 의 `pageRefTitles` — 참조 id → 평문, 볼 수 없으면 `null`)으로 따로 준다. 화면은 볼 수 없는 참조를 "접근 권한 없음"으로 그리고 열지 않는다. 볼 수 없는 참조도 자리는 남는다 (#81) | ① **읽을 때만 가리면 안 된다**: 가린 문서를 PUT 으로 되보내면 `updateYFragment` 가 Y.Doc 의 제목을 빈 값으로 덮고, 협업 참여자는 Y.Doc 을 그대로 받는다 — 제목이 노드에 있는 한 누가 볼 수 있는지와 무관하게 퍼진다 ② **attr 을 비우지 않고 없앴다**: 남기면 누군가 다시 채우거나 그린다(`toDOM` 이 그렸다 — 클립보드 HTML). 없애면 어느 경로도 쓸 수 없다. 옛 Y.Doc 의 attr 은 prosemirror-model 1.25.11 의 `NodeType.create` 가 무시하고(던지는 것은 `createChecked` · `check()` 의 `checkAttrs` 뿐 — 소스) 다음 본문 저장이 지운다 — 실제 `ySyncPlugin` 바인딩까지 검사가 본다 ③ **자리는 남기고 제목만 가린다**: 참조의 자리(id · 순서)는 이미 본문에 보이고 익스포트도 자리만 센다(§3.3-74). 자리를 빼면 편집기가 참조 없는 문서를 저장해 `page_ref_missing` 으로 거부된다 ④ **맵은 목록과 같은 규칙**(`perm_scope_id = ANY(readableScopes)` · §3.3-32) — #80 뒤로 스코프가 정의와 맞는다 ⑤ 부수 효과: 하위 페이지 이름을 바꿔도 부모의 참조가 낡지 않는다. 먼저 실패하는 검사 3개로 재현했다. 반사실: `rowsToDoc` 이 제목을 실으면 ① 과 본문 저장 검사 2개 · 맵이 권한을 안 거르면 ① 1개 · schema 에 `title` 을 되살리고 어댑터가 실으면 ②(본문 저장) · ③ · `/page` 3개 · attr 만 되살리면(쓰는 곳 없이) ③ · `/page` 2개 — 각각 그 주장의 검사만 실패. 마지막이 "비우기가 아니라 없애기"를 가려낸다 — attr 이 schema 에 있으면 저장이 빈 값을 써 attr 이 남는다 |
| 23 | **참여자가 하위 페이지 참조를 깊이 상한을 넘는 자리로 옮기면 어떻게 하는가** (정본은 `MAX_TREE_DEPTH` "100, 초과 시 명시적 에러" `[확인필요]` 까지만 적었다. §7 이 후보로 "되돌리는 수선 update · 행 자리 유지"를 남겼고 §3.3-98 이 5d 의 선결 조건으로 적었다) | **참여자 경로는 거부하지 않고 들어갈 수 있는 깊이까지 한 단씩 올린다** — 정규화 규칙 `page_ref_lifted`(부모의 바로 뒤 형제로)를 받은 깊이(`pageRefDepthLimits` — 투영하는 트랜잭션의 행에서 잰다)가 있을 때만 적용하고, **투영과 수선이 같은 받은 깊이로 읽는다.** 본문 저장(PUT) · 명령은 계속 거부한다. 모든 경로에서 **더 깊어지지 않는 이동은 거부하지 않는다**(`relocateSubtree`). 참조의 부모가 그대로여도 **경로가 바뀌면 옮긴 것이다.** 본문에 모르는 노드가 있어 수선을 쓸 수 없을 때만 거부한다 (#82) | ① **되돌리기 · 행 자리 유지가 아니라 올리기**: 되돌릴 옛 자리는 Y.Doc 에 없고(행의 `order_key` 뿐) 같은 update 가 그 자리를 지웠을 수 있다. 행 자리 유지는 "행 = Y.Doc 투영"(4b)을 깨고 다음 투영이 같은 이동을 또 만난다. 올리기는 문서와 서브트리 높이만으로 정해지는 결정론 함수이고, 정규화의 "깊이 상한을 넘는 자식은 올린다"(`children_lifted`)와 같은 모양이다 — 사용자가 둔 자리에서 가장 가까운 곳에 남는다 ② **투영과 수선이 같은 받은 깊이**: 투영만 올리면 행과 Y.Doc 이 다른 자리다. 올린 Y.Doc 은 받은 깊이 없이 읽어도 같은 문서라(올린 뒤에는 고칠 것이 없다 — `normalize.test.ts` ②-1 이 두 방식으로 다시 읽어 본다) 다른 읽는 곳(협업 참여자 · `loadDocState` · 검사)은 바꾸지 않았다. 받은 깊이는 행에 있는 서브트리 높이라 문서만 보는 곳은 알 수 없다 ③ **더 깊어지지 않는 이동은 거부하지 않는다**: 본문은 페이지에서 센 깊이만 막아(`validateDoc` · 정규화) 상한을 넘은 서브트리가 정상 편집으로 생긴다(진단: 깊이 1 페이지 아래 경로 101). 그 페이지를 같은 깊이 · 더 얕은 곳으로도 옮길 수 없었다. 이 규칙으로 받은 깊이가 늘 1 이상이 되어 **최상위까지 올리면 반드시 들어간다** — 올리기에 끝이 있다. 상한을 넘은 서브트리의 참조는 지금 깊이까지만 올린다 ④ **경로가 바뀌어도 이동**: 진단에서 참조를 담은 토글을 옮기면 하위 페이지 서브트리의 경로가 옛 자리에 남았다(X-7) — 깊이 검사도 건너뛰어 올리기가 토글째 옮기는 경우를 놓친다 ⑤ **모르는 노드에서만 거부**: 수선은 매핑 없이 비교해 모르는 요소를 지우므로 쓰지 않는다(§3.3-91). 올린 것을 쓰지 못하면 행과 Y.Doc 이 다른 자리다. 새 버전 클라이언트만 넣으므로 스키마 버전 게이트(§7)가 오면 닿지 않는다 ⑥ **정본의 "명시적 에러"는 그대로다**: 에러를 돌려줄 수 있는 경로(PUT · 명령 · 이동 API)는 거부한다. 에러를 돌려줄 수 없는 CRDT 경로만 올린다 ⑦ **동시성**: 올리기와 동시에 담은 블록 · 형제 · 이웃에 친 글자는 남는다(`repair.test.ts` ⑤ — 옮기는 것은 글자 없는 참조 요소뿐). 같은 참조를 동시에 다른 곳으로 옮기면 참조가 둘이 되고 둘째가 유령 페이지가 된다 — 올리기와 무관하게 두 참여자만으로 생기는 기존 버그다(§7). 반사실: 이동 규칙을 되돌리면 이동 검사와 "지금 깊이까지만" 2개 · 부모만 비교하면 PUT 경로 검사 1개(§3.3-111) · 올리지 않으면 참여자 검사 3개와 협업 ⑬ · 수선이 받은 깊이를 안 넘기면 같은 4개 · 모르는 노드 선검사를 빼면 1개 · 받은 깊이 식의 뒷항(지금 깊이)을 빼면 1개 · 경계를 한 칸 당기면(`>=`) 정규화 2개와 참여자 3개와 ⑬ · 수선이 받은 깊이를 무시하면 수선 ⑤ 2개와 참여자 3개와 ⑬ · 컨테이너 앞으로 올리면 정규화 · 수선 · 참여자 · ⑬ 8개가 — 각각 그 주장의 검사만 실패. `commit` 의 던지기 · 더 깊어지는 참조만 재기는 반사실이 통과했다(§3.3-110) |
...
| 101 | **퍼뜨리기를 커밋 신호 하나에 묶었다 — 참여자 update 도 쌓은 뒤 자기 신호를 기다린다** (#79) | 5a 는 쌓은 직후 꼬리를 직접 적용했다. 5c 에서 명령 · 권한의 신호가 따로 오게 되자 두 길이 서로 앞지를 수 있다 — 참여자 update 를 직접 적용하면 그보다 먼저 커밋된 회수의 신호가 아직 오지 않았어도 퍼진다. 퍼뜨리는 곳이 신호 하나면 그 순서가 곧 커밋 순서다. 대가는 update 마다 신호 한 번의 왕복. ⚠ 신호를 듣는 연결이 죽어 있으면 참여자 update 는 다시 붙을 때까지 기다린다 — 쌓기는 이미 끝났으므로 잃지는 않는다. 반사실: 쌓자마자 직접 적용하면 ⑩ 의 전제("신호를 붙잡은 동안에는 아무것도 퍼지지 않는다")만 실패 |
| 102 | **꼬리는 신호가 알린 seq 까지만 · 다시 판정이 끝난 뒤에 — 하나만으로는 회수를 앞지른다** (#79) | 판정만 기다리면: 본문 N 의 신호로 꼬리를 끝까지 읽을 때 이미 커밋된 N+1(회수 R 뒤)까지 읽어 R 의 신호보다 먼저 퍼뜨린다. seq 상한만 두면: R 의 신호가 먼저 와도 판정이 끝나기 전에 N+1 의 적용이 달려 나간다. 설계 중 코드를 다시 읽다 앞의 구멍을 찾아 상한을 넣었다. 검사는 신호를 붙잡아 두었다가 회수 전 명령의 신호까지만 놓는다(`holdableFeed`) — 신호가 늦게 오는 장면은 기다려서는 만들 수 없다. 반사실: 끝까지 읽으면 "신호가 알린 seq 를 넘어 읽었다" · 판정을 기다리지 않으면 "회수 뒤에 커밋된 명령이 회수된 연결에 갔다" — 각각 ⑩ 의 그 단언만 실패 |
| 103 | **⑪ 은 처음에 반사실을 가려내지 못했다 — 빈 페이지라 받은 것이 보이지 않았다** (#79) | "첫 동기화 전에 다시 판정한다"를 빼도 통과했다. 원인: 비밀 페이지 본문이 비어 본문을 받았는지 볼 수 없었고, 늦게 붙은 쪽이 곧이어 보내는 SyncStep2 가 쓰기 경로(`appendDocUpdate` 의 권한 검사)에서 같은 이유 `not_found` 로 닫혀 닫힘 이유까지 같았다. 주장은 맞으므로 검사를 고쳤다(§5): 본문을 저장한 뒤 로그 · 스냅샷을 지워 옮기기 전 페이지로 만들고(doc-store 검사의 선례), 같은 페이지의 스냅샷 행을 **커밋하지 않고** 넣어 불러오기가 권한 검사를 지난 뒤 옮기기 INSERT 에서 그 트랜잭션을 기다리게 했다 — `pg_locks` 로 기다림을 확인한다. 표 잠금과 달리 다른 검사 파일을 막지 않는다. 이제 그 반사실에서 "회수된 연결이 본문을 받았다"만 실패 |
| 104 | **반사실이 통과해 주석을 고친 것 둘 — 신호 버퍼 · 불러온 뒤 꼬리 읽기** (#79) | ① 다시 붙은 연결의 신호를 `onResync` 뒤에 넘기는 버퍼(`change-feed.ts`) ② 문서를 불러와 등록한 뒤 꼬리를 한 번 읽는 것(`afterLoadDocument`). 둘 다 빼도 검사가 전부 통과했다 — LISTEN 응답과 같은 묶음에 신호가 실려 오는 장면, 불러오기와 등록 사이에 커밋을 끼우는 장면을 만들지 못했다. 코드는 두고(순서 추론으로는 필요하다) "검사로 강제하지 못한 방어"로 주석을 고쳤다 |
| 105 | **④ 는 신호를 붙잡은 채로 돈다** (#79) | 5c 뒤에는 권한 신호가 강등 · 세션 폐기를 받자마자 연결을 닫는다. 그러면 ④ 의 편집이 도착하기 전에 같은 이유로 닫혀, 쓰기 경로가 세션 · 권한을 다시 묻는지 가려낼 수 없다(추론 — 5a 의 반사실을 다시 돌리지는 않았다). 신호를 붙잡은 채 강등 · 폐기 · 편집을 하고, 닫힘을 본 뒤 놓는다 |
| 106 | **트리거를 하나씩 끄는 반사실에서 검사 둘이 트리거를 가려내지 못했다 — 신호가 겹쳐 울렸다** (#79) | ① 커밋 신호 검사: `acl_entry` 트리거를 꺼도 통과했다 — 첫 부여는 스코프 갱신(블록 트리거)이, 상속 끊기는 메타 행이 **같은 페이로드**를 함께 보내 가려졌다. 레벨만 바꾸는 부여(ACL 행 하나만 바뀐다)를 더했다. 같은 이유로 상속 되받기(메타 행만)와 세션 행 삭제도 더했다 ② 협업 ⑨: `acl_entry` · `user_session` · `workspace_member` 를 꺼도 통과했다 — **다시 판정이 워크스페이스 단위라**, 쓰기를 한꺼번에 하면 뒤의 쓰기(멤버 제외 · 휴지통)가 보낸 신호가 앞의 연결까지 닫았다. 쓰기 하나마다 그 연결이 닫히기를 기다리고 다른 연결은 그대로인지 보게 고쳤다. 주장(각 쓰기가 신호를 보낸다)은 맞았고 검사가 못 가려냈다(§5) |
| 107 | **이동 검사가 틀린 동작을 단언하고 있었다 — 그리고 알려진 누출 하나를 재다 구멍 둘이 더 나왔다** (#80) | `move-page.db.test.ts` 의 "루트 아래로 들어가면 서브트리 전체가 새 스코프를 받는다"는 W5-a(ACL 이 없던 때)의 검사다. W6-b 가 루트에 `workspace_everyone` 행을 넣어 옮긴 루트가 경계가 된 뒤에도 그대로 통과했다 — 코드의 주석("경계는 최상위뿐")과 검사가 같은 전제에 함께 남았다. "본문 블록의 스코프도 함께 옮겨간다"도 루트를 옮겨 같은 전제였다. 둘 다 경계가 아닌 하위 페이지를 옮기게 고쳤다. #79 가 적은 누출 하나만 재려던 진단을 같은 함수의 네 장면(비공개 → 공개 밑 · 최상위로 꺼내기 · 루트 → 비공개 밑 · B4 복원)으로 넓히자 **아무도 못 보는 페이지**와 **사이드바에서 사라지는 페이지**가 함께 나왔다. 검사는 같은 페이지에 권한 판정(`canViewPage`)과 목록(사이드바 · 하위 목록 · 검색)을 **나란히** 묻는다 — 둘이 어긋나는 것이 이 버그의 모양이다 |
| 108 | **흔들리던 검색 오버레이 e2e 검사의 원인은 이미지 노드 뷰의 비동기 오류 문구였다 — 넣어 둔 진단이 잡았다** (#81) | W8-b 부터 "오버레이가 열린 동안의 입력이 본문에 새지 않았다"가 가끔 떨어졌다. 원인을 추측하지 않고 실패하면 달라진 글자를 남기게 해 두었고(§7), #81 의 실행에서 `…참조 제목 …` → `…참조 제목 …이미지를 불러올 수 없습니다. 원본 열기` 가 찍혔다 — §7 이 확인하지 않은 후보로 적은 그대로다. 본문 전체의 `textContent` 대신 **편집 불가 노드 뷰(`contenteditable=false`)를 뺀 글자**를 비교한다 — ProseMirror 가 내용 DOM 없는 노드 뷰에 달고(소스) 이미지 뷰는 직접 단다. 새는 타이핑은 편집 가능한 블록으로 들어가므로 검사의 목적은 그대로다. e2e 반사실(포커스 이동 끄기)은 다시 돌리지 않았다. 같은 실행에서 새 절의 전제 검사가 `pages/{id}` 의 GET(라우트에 없다 — 405)으로 재서 실패했다 — 뒤따른 ★ 는 통과해 권한은 막혀 있었고, 재는 방법이 틀렸다. 권한을 거치는 본문 GET 으로 바꿨다 |
| 109 | **깊이 초과를 풀기 전에 진단으로 쟀더니 같은 투영의 구멍 둘과 기존 버그 하나가 더 나왔다** (#82) | HANDOFF 가 "후보를 진단으로 먼저 잰다"고 적었다. 진단 스크립트 셋으로 ① 참여자 경로가 정말 `page_ref_too_deep` 로 거부하는지 ② 상한을 넘은 서브트리가 생기는지 · 그걸 옮길 수 있는지 ③ 참조를 담은 토글을 옮기면 하위 페이지 경로가 따라가는지를 쟀다. ②는 **깊이 1 페이지 아래 경로 101** 이 본문 저장만으로 생겼고 같은 깊이 · 더 얕은 이동이 모두 `too_deep` 였다. ③은 토글 행만 따라가고 하위 페이지 서브트리는 옛 경로에 남았다. 올리기의 동시성(HANDOFF 가 짚은 "수선과 동시에 옮긴 블록에 친 글자")을 재다 **같은 참조를 둘이 동시에 옮기면 참조가 둘이 된다**는 것을 보고, DB 투영까지 따라가 **제목 없는 살아 있는 `page` 행**이 생기는 것을 확인했다. PUT 에 모르는 id 의 참조를 넣어도 같고, 다른 곳의 페이지를 참조하면 PK 위반이다. ②③은 올리기의 전제라 함께 고쳤고(먼저 실패하는 검사로 재현), 유령 페이지는 경계가 달라 §7 · §8 ① 로 남겼다 |
| 110 | **반사실이 통과해 주석을 고친 것 둘 — `commit` 의 던지기 · 더 깊어지는 참조만 재기** (#82) | ① `doc-store.ts` `commit` 은 올린 참조를 수선으로 쓰지 못하면 던진다. 호출자(`body-write.ts` `finish`)가 모르는 노드를 먼저 걸러 닿는 입력이 없어 빼도 검사가 전부 통과했다 — "검사로 강제하지 못한 방어"로 적었다(선검사를 빼는 반사실은 모르는 노드 검사가 잡는다) ② `pageRefDepthLimits` 는 더 깊어지는 참조만 서브트리 높이를 잰다. 더 깊어지지 않는 참조는 받은 깊이 식의 뒷항으로 늘 들어가 모두 재도 결과가 같다 — "빠른 길일 뿐"으로 적었다(§3.3-95 와 같은 처리) |
| 111 | **"경로가 따라간다"는 참여자 검사가 아니라 PUT 검사가 고정한다** (#82) | 투영이 부모만 비교하게 되돌리는 반사실에서 참여자 검사 "담은 토글을 깊이 옮겨도 같다"는 **통과했다.** 그 장면에서는 올리기가 참조를 토글 밖으로 빼 부모가 바뀌므로 부모 비교만으로도 이동으로 잡힌다. 투영 코드는 두 경로가 같으므로 PUT 검사(`save-page-body.db.test.ts` "참조를 담은 블록을 다른 블록 밑으로")가 그 주장을 고정한다. 올리지 않는 참여자 장면(깊이 안에서 토글째 옮기기)은 따로 쓰지 않았다 |
...
| 항목 | 상태 | 어디 |
|---|---|---|
| ~~자기 ACL 을 가진 페이지를 옮기면 `perm_scope_id` 가 대상의 스코프로 덮인다 — 목록 · 사이드바가 볼 수 없는 페이지의 제목을 내준다~~ | **해결(#80 · §3.2-21).** 진단을 넓히자 같은 함수의 구멍 둘이 더 나왔고 함께 막았다 — 최상위로 꺼내거나 B4 로 되살린 페이지를 아무도 못 봤고, 볼 수 있는 페이지가 사이드바에서 사라졌다. 아래는 #79 때 적은 것이다 — `relocateSubtree` 가 옮긴 페이지의 스코프를 늘 `target.perm_scope_id` 로 쓴다 — 주석이 "지금은 acl_entry 가 없으므로 경계는 워크스페이스 최상위뿐"(W5-a)이라고 적은 채 W6-b 뒤에도 그대로다. 정의(§3.11)로는 자기 ACL 을 갖거나 상속을 끊은 페이지는 **자기가 스코프**다. 재현: 상속을 끊고 소유자만 남긴 페이지를 모두에게 열린 페이지 밑으로 옮기면 멤버는 볼 수 없는데(`canViewPage` false) `listChildPages` · `listPageTree` 가 그 제목을 내주고, `search_document.perm_scope_id` 도 대상 스코프가 된다(검색 질의로는 재지 않았다 — 같은 규칙으로 거르므로 걸릴 것이다). 옛 스코프를 따르던 자손도 대상 스코프로 간다. `move-page.db.test.ts` 의 "루트 아래로 들어가면 서브트리 전체가 새 스코프를 받는다"가 **이 동작을 단언한다**(옮긴 루트는 `workspace_everyone` ACL 을 가진 경계다). 복원의 B4 재부모화 · 프로젝터의 자리 옮기기도 같은 함수다 | `src/lib/block/move-page.ts` `relocateSubtree` · `move-page.db.test.ts` |
| ~~부모 본문의 하위 페이지 참조가 볼 수 없는 페이지의 제목을 싣는다~~ | **해결(#81 · §3.2-22).** 참조 노드에서 제목을 뺐다 — schema 의 `page_ref` 에 `title` attr 이 없어 어느 경로도 Y.Doc 에 제목을 쓸 수 없고, 화면은 본문을 받을 때 권한으로 거른 제목 맵(`pageRefTitles`)을 따로 받아 볼 수 없는 페이지를 "접근 권한 없음"으로 그린다. 아래는 원래 적었던 것이다 — `loadPageBody`(`rowsToDoc`)는 본문의 하위 페이지 참조에 그 페이지 행의 제목을 권한과 무관하게 싣고, Y.Doc 의 참조 노드에도 만들 때의 제목(`title` attr)이 들어 있다 — 부모를 볼 수 있으면 비공개 하위 페이지의 제목이 편집기 · 협업 참여자에게 보인다. **읽을 때만 가리면 안 된다**: 가린 문서를 본문 저장(PUT)으로 되보내면 `updateYFragment` 가 Y.Doc 의 참조 제목을 빈 값으로 덮고, 협업 참여자는 Y.Doc 을 그대로 본다. 참조 노드에서 제목을 빼고 화면이 권한을 거쳐 읽는 쪽(노션처럼 "접근 권한 없음" 자리 표시)이 후보다 — 에디터 바인딩(6조각) 전에 정한다. 익스포트는 이미 뺀다(§3.3-74) | `src/lib/editor/document.ts` `rowsToDoc` · `block/page-refs.ts` · `save-page-body.ts` `readScope` |
...
| **B4 의 Private 루트** | **부분 해결(#80 · §3.2-21).** 부모가 사라진 페이지는 여전히 **워크스페이스 최상위**로 되살리되(Private 루트가 없다), **되살린 사람에게만** `full_access` 를 줘 정본의 Private 루트처럼 동작한다 — 전에는 아무 행도 넣지 않아 아무도 못 봤다. 남은 것: 사이드바에 Private 섹션이 없어 "내 페이지"로 구분되지 않는다 · 명시 부여를 지우지 않는다(정본 `move_to_private` 과 다르다) · 되살린 사람에게 이미 낮은 레벨의 행이 있을 때 올리는 경로에 검사가 없다 | `trash.ts` `restorePage` · `permissions/acl.ts` `grantToRestorer` |
| **#80 이전에 옮기거나 B4 로 되살린 페이지의 스코프 · 최상위 행은 고쳐지지 않았다** | 코드만 고쳤고 이미 쓰인 행을 다시 계산하는 마이그레이션을 쓰지 않았다 — 운영 데이터가 없고 검사 DB 는 매번 새 워크스페이스를 만든다. 개발 DB 가 이상하면 `npm run db:reset`. 운영 데이터가 생기기 전에 옮기려면 정의(자기 또는 가장 가까운 경계 조상)대로 `perm_scope_id` 를 다시 계산하고 ACL 이 없는 최상위 노드에 행을 넣는 SQL 을 **검사와 함께** 쓴다(적용한 마이그레이션은 고칠 수 없다) | `db/migrations/` |
| **최상위 페이지를 다른 페이지 밑으로 옮겨도 워크스페이스 전체에 열려 있다** | 최상위 페이지의 `workspace_everyone → full_access` 행이 옮긴 뒤에도 남는다(§3.2-21 ④). F-06-20 은 "이전 부모 기반 접근은 소멸"이라 적었지만 이 행은 명시 부여와 구분되지 않는다(아래 "ACL provenance"). 이동 경고(아래)도 없어 사용자는 모른다 | `permissions/acl.ts` `inheritFromWorkspace` · `block/move-page.ts` |
...
| **수선과 동시에 옮긴 블록에 친 글자** | 옮기는 수선(그룹 합치기 · 자식 올리기)은 옮긴 블록을 새 요소로 만든다. 수선과 **동시에** 그 블록에 친 글자는 지워진 옛 요소에 들어가 사라진다 — 동시 순서 변경과 같은 한계. 위반이 생긴 append 가 곧바로 고치므로 창은 짧다 — 재지 않았다. 하위 페이지 참조를 올리는 수선(#82)은 글자 없는 참조 요소만 옮겨 여기에 걸리지 않는다(`repair.test.ts` ⑤) | `src/lib/collab/repair.ts` |
...
| ~~참여자 경로의 깊이 초과는 아직 거부한다~~ | **해결(#82 · §3.2-23).** 참여자 update 는 들어갈 수 있는 깊이까지 올리고 그 수선을 같은 seq 에 쌓는다. **남은 것**: 본문에 모르는 노드가 있으면 올린 것을 Y.Doc 에 쓸 수 없어 여전히 `page_ref_too_deep` 로 거부한다(스키마 버전 게이트 · 아래). 본문 저장(PUT) · 명령은 의도적으로 계속 거부한다 | `src/lib/block/body-write.ts` `pageRefDepthLimits` |
| **본문의 하위 페이지 참조가 이 본문의 자식이 아닌 id 를 가리키면 투영이 페이지 행을 만들거나 던진다 — 유령 페이지** | #82 의 진단에서 찾았다. 투영(`projectRows`)은 문서의 `type='page'` 블록 중 범위에 없는 것을 **본문 블록처럼 넣는다.** ① 본문 저장(PUT)에 모르는 uuid 의 참조를 넣으면 제목 없는 살아 있는 `page` 행이 생긴다 — `createPage` 를 거치지 않아 `create_child` · 검색 색인 · 본문이 없다 ② 다른 곳의 페이지를 참조하면 PK 위반으로 던진다(500 · 참여자면 연결이 오류로 닫힌다) ③ **같은 참조를 둘이 동시에 다른 곳으로 옮기면**(y-prosemirror 에 옮기기가 없어 둘 다 새 요소를 넣는다) 정규화가 둘째에 파생 id 를 주고 ① 이 된다 — 에디터가 붙으면 드래그 두 번으로 생긴다. 5d 전에 규칙을 정한다 — 투영을 떼면 던질 수 없다(후보: 범위에 없는 참조는 수선이 Y.Doc 에서 뺀다 · PUT 은 거부) | `src/lib/block/save-page-body.ts` `projectRows` · `src/lib/editor/document.ts` `validateDoc` |
...
| ~~명령이 쓴 것은 다음 참여자 update 때까지 연결된 참여자에게 가지 않는다~~ | **해결(#79 · §3.2-20).** 쓰는 트랜잭션의 트리거가 커밋 신호(NOTIFY)를 보내고 협업 서버가 그 seq 까지 꼬리를 적용한다 — 누가 편집하기를 기다리지 않는다(`collab-server.db.test.ts` ⑧) | `src/lib/collab/collab-server.ts` · `change-feed.ts` · `db/migrations/0016_collab_notify.sql` |
| ~~권한이 줄어도 서버가 먼저 끊지 않는다~~ | **해결(#79 · §3.2-20).** 권한 판정이 읽는 표(ACL · 상속 · 페이지 경로 · lifecycle · 멤버 · 세션 · SSO)의 쓰기가 권한 신호를 보내고, 협업 서버가 그 워크스페이스(세션이면 그 사용자)의 연결을 다시 판정해 닫는다 — 강등 `forbidden` · 볼 수 없게 됨 `not_found` · 세션 `unauthenticated`(⑨). 회수가 커밋된 뒤에 커밋된 본문은 회수된 연결에 가지 않는다(⑩). **남은 것**: 볼 수만 있던 연결은 권한이 올라가도 다시 붙기 전까지 읽기 전용이다(아래) | 같은 곳 |
| **볼 수만 있던 연결은 권한이 올라가도 읽기 전용 그대로** | 5c 는 줄어든 것만 닫는다. 올라간 연결을 닫으면 읽던 사람의 화면이 끊기고, 읽기 전용 플래그를 풀면 그 연결이 읽기 전용 동안 로컬에 쌓아 두고 서버가 버린 update 에 기대는 다음 update 가 `missing_dependencies` 로 닫힌다. 다시 열면 고칠 수 있다 — 6조각에서 화면이 "권한이 바뀌었다"를 보여 줄지 정한다 | `src/lib/collab/collab-server.ts` `connectionVerdict` |
| **세션이 시간이 지나 만료되는 것은 알리지 않는다** | 세션 폐기(`revoked_at`)와 만료 시각 변경은 신호를 보내지만, 쓰기 없이 `expires_at` 이 지나가는 것은 DB 이벤트가 없다. 그 연결은 다음 쓰기 · 다음 첫 동기화 · 같은 워크스페이스의 다음 권한 신호에서 닫힌다(모두 세션을 다시 해석한다). F-05-19 의 "토큰 만료 → 재연결 유도"는 만료 시각에 맞춘 타이머나 주기적 다시 판정이 필요하다 | `src/lib/collab/collab-server.ts` · `0016_collab_notify.sql` |
| **권한 신호는 워크스페이스 단위다 — 정밀 무효화가 아니다** | 한 페이지의 ACL 을 바꿔도 그 워크스페이스에 붙은 연결을 전부 다시 판정한다(연결마다 세션 해석 1 + 권한 판정 1 트랜잭션). 05 F-05-19 의 클론 대안 (c) 이고 서브트리 무효화는 v2 다. 멤버가 많은 워크스페이스에서 공유 패널을 연달아 누르면 그만큼 돈다 — 재지 않았다 | 같은 곳 |
| **권한을 다시 판정하지 못하면 닫는 경로에 검사가 없다** | 판정 중 DB 오류면 `unavailable` 로 닫는다(회수를 모른 채 두지 않는다). 판정 중에 DB 오류를 끼우는 검사를 쓰지 않았다 | 같은 곳 `recheck` |
| **협업 서버는 한 대다** | 연결 · 메모리 문서가 프로세스 안에 있다. 5c 부터 본문은 커밋 신호로 서버를 넘는다 — 서버마다 LISTEN 해 자기 메모리 문서를 로그 꼬리로 맞추므로 두 대여도 본문은 퍼질 것이다(**두 대로 검사하지 않았다**). awareness(커서 · 참여자 목록)는 서버를 넘지 않는다(정본: Redis pub/sub 등 외부 브로커). 신호를 듣는 Postgres 연결은 서버마다 하나다 | 같은 곳 · `change-feed.ts` |
| **협업 서버는 Hocuspocus 내부 동작에 기댄다** | `beforeSync` 가 update 를 **적용하기 전에** 불려 끝나기를 기다린다는 것, 거기서 던지면 그 문서 연결만 닫힌다는 것(4.7.0 소스 · 진단). 5c 부터는 SyncStep1 도 `beforeSync` 를 거친다는 것 · 읽기 전용 연결의 update 를 닫지 않고 버린다는 것 · `Connection.close` 가 그 문서 연결만 닫는다는 것에도 기댄다. 올릴 때 `collab-server.db.test.ts` ④ · ⑦ · ⑨ · ⑪ 이 먼저 깨진다 | 같은 곳 · `package-lock.json` |
...
| ~~하위 페이지 제목을 바꿔도 부모 본문의 참조 노드 title 은 그대로~~ | **해결(#81).** 참조 노드에 제목이 없다 — 제목은 본문을 받을 때 행에서 읽는다. **남은 것**: 편집기를 연 동안 다른 탭에서 이름을 바꾸면 다시 불러올 때까지 옛 이름이다. 6조각이 협업 편집기의 제목 맵을 어디서 받을지(본문 GET 을 한 번 부르거나 따로 라우트) 정한다 | `src/app/w/[workspaceId]/[pageId]/body-editor.tsx` |
| **옛 Y.Doc 의 참조 노드에 남은 `title` attr** | #81 전에 만든 본문 Y.Doc 은 참조 요소에 `title` attr 이 남아 있다. 다음 본문 저장(PUT)에서 사라진다(`page-ref-titles.db.test.ts` ③ — `updateYFragment` 가 schema 에 없는 attr 을 지운다). 바인딩의 로컬 편집도 지우는지는 확인하지 않았다(③ 은 그 편집이 참조를 지우지 않는 것까지만 본다). **그 전에 협업 서버에 붙는 참여자는 동기화로 그 제목을 받는다.** 쓰기 없이 지우는 경로(Yjs update 를 만드는 복구)는 만들지 않았다 — 운영 데이터가 없다. 운영 전에 옮기려면 모든 페이지 본문을 한 번 열어 `title` attr 을 지우는 update 를 쌓는 스크립트를 검사와 함께 쓴다 | `src/lib/editor/schema.ts` · `collab/doc-store.ts` |
| **공유 API 가 워크스페이스에 없는 사용자에게도 부여를 받는다** | `POST /pages/{id}/access` 의 `grant` 는 사용자 id 가 uuid 이기만 하면 받는다 — 멤버인지 보지 않는다(#81 의 e2e 가 이것으로 "소유자도 못 보는 하위 페이지"를 만든다). 새는 것은 없지만(그 사용자는 워크스페이스에 들어올 수 없다) 공유 패널이 모르는 주체가 남는다. 외부인 초대(F-06-07)가 올 때 게스트 좌석과 함께 막는다 | `src/app/api/workspaces/[workspaceId]/pages/[pageId]/access/route.ts` |
...
끝난 것 — **본문의 정본이 행에서 Y.Doc 으로 넘어갔다**(4b). 본문 행은 Y.Doc 의 투영이고, 편집기는 아직 Phase 0 그대로(PUT 본문 저장)다:
  ...
  - 5c조각(#79) db/migrations/0016_collab_notify.sql · src/lib/collab/change-feed.ts — 쓰는 트랜잭션의 트리거가 NOTIFY 로 커밋 신호를
    보내고(collab_doc · collab_access) 협업 서버가 LISTEN 한다. 퍼뜨리기는 신호를 커밋 순서대로 따라 한 곳에서 — 권한 신호를 받으면
    그 워크스페이스 연결을 다시 판정해 닫은 뒤에야 다음 꼬리를, 신호가 알린 seq 까지만 적용한다. 강등은 forbidden 으로 닫는다(읽기
    전용 전환 아님) · 첫 동기화 전에 한 번 더 판정 · 끊겼다 붙으면 전부 다시 맞춘다(§3.2-20, 다시 판단하지 마라)
  - 5c 뒤(#80) src/lib/block/move-page.ts(relocateSubtree) · permissions/acl.ts — 이동 · 복원의 권한 스코프. 경계(자기 ACL · 상속
    끊기)인 페이지는 옮겨도 자기 스코프를 지키고, 최상위로 오면 상속 원천을 ACL 행으로 둔다 — 옮기기는 워크스페이스에서
    상속(inheritFromWorkspace, 만들 때와 같은 함수), B4 복원은 되살린 사람만(grantToRestorer)(§3.2-21, 다시 판단하지 마라)
  - (#81) src/lib/editor/schema.ts · block/save-page-body.ts(loadPageBody) — 하위 페이지 참조 노드는 제목을 싣지 않는다(title attr
    없음). 제목은 권한으로 거른 pageRefTitles 로 따로 주고 편집기는 볼 수 없는 참조를 "접근 권한 없음"으로 그린다(§3.2-22, 다시
    판단하지 마라)
  - (#82) src/lib/collab/normalize.ts(page_ref_lifted) · block/body-write.ts(pageRefDepthLimits) — 참여자가 하위 페이지 참조를 그
    서브트리가 깊이 상한을 넘는 자리로 옮기면 거부하지 않고 들어갈 수 있는 깊이까지 올린다. 투영과 수선이 같은 받은 깊이로 읽는다.
    더 깊어지지 않는 이동은 거부하지 않고(relocateSubtree), 참조를 담은 블록을 옮기면 하위 페이지 경로가 따라간다(§3.2-23, 다시
    판단하지 마라)

다음 작업 — 순서대로:
  ① **본문 참조가 만드는 유령 페이지를 막는다**(§7 "본문의 하위 페이지 참조가 이 본문의 자식이 아닌 id 를 가리키면") — 5d 의 선결
     조건이다. 투영이 범위에 없는 type='page' 블록을 본문 블록처럼 넣어 제목 없는 살아 있는 페이지 행을 만들거나(모르는 id) PK 위반으로
     던진다(다른 곳의 페이지). 같은 참조를 둘이 동시에 옮기면 정규화가 둘째에 파생 id 를 줘서 생긴다 — 에디터가 붙으면 흔해진다.
     참여자 경로는 거부할 수 없고(§3.2-19 ②) 5d 뒤에는 투영이 던질 수도 없다. 먼저 실패하는 검사로 셋을 재현하고, 후보(범위에 없는
     참조는 수선이 Y.Doc 에서 뺀다 · PUT 은 거부)를 진단으로 잰다 — 뺀 참조가 가리키던 진짜 페이지의 행은 그대로인지 본다
  ② 5d(투영 디바운스 · projected_seq) — 정본 프로젝터 SLO(p95 < 2s · 쓰기 증폭). §2 "5d 가 지켜야 할 것 — #82" 를 먼저 읽는다
  ③ 6조각(에디터 바인딩) — 지켜야 할 것이 §2 의 3a · 5a · 5c 항목에 쌓여 있다(참조 제목 맵 포함)
  ④ 착수 전에 읽을 것: §7 "본문의 하위 페이지 참조가" · §3.2-19 · §3.2-23 · src/lib/block/save-page-body.ts(projectRows 의 toInsert ·
     pageRefMoves) · src/lib/collab/normalize.ts(duplicate_id) · src/lib/collab/repair.ts 머리말 · §3.2-14
...
지켜야 할 것:
  ...
  - perm_scope_id 는 relocateSubtree(이동 · 복원 · 프로젝터)와 acl.ts 의 rescope 밖에서 쓰지 마라 — 목록(스코프)과 권한 판정(조상
    사슬)이 다르게 답한다. 노드를 최상위에 두는 새 경로는 acl.ts 의 inheritFromWorkspace(B4 는 grantToRestorer)를 부른다 — ACL 이
    없는 최상위 노드는 아무도 못 본다(§3.2-21). 권한 검사는 canViewPage 와 목록(사이드바 · 하위 목록 · 검색)을 나란히 묻는다
  ...
  - 구조 위반을 Y.Doc 에 고쳐 쓰는 곳은 appendDocUpdate 하나다. 에디터 · 협업 서버에 정규화 appendTransaction 을 두지 마라 —
    둘이 각자 고치면 옮긴 블록이 복제된다(repair.test.ts ②). 협업 서버도 수선을 따로 퍼뜨리지 않는다 — 로그 꼬리를
    메모리 문서에 적용하는 것이 퍼뜨리기다(collab-server.ts)
  - 정규화가 문서 밖의 사실(하위 페이지 참조가 놓일 수 있는 깊이 — pageRefDepth)을 받으면 투영의 읽기와 수선에 **같은 값**을
    넘긴다 — 한쪽만 넘기면 행과 Y.Doc 이 다른 자리다. 깊이 초과는 참여자 경로에서 올리고 PUT · 명령에서는 거부한다(§3.2-23)
  - 협업 서버에서 메모리 문서에 update 를 적용하는 곳은 **커밋 신호를 따르는 꼬리 적용 하나**다. 참여자 update 를 쌓자마자
    직접 적용하지 마라 — 먼저 커밋된 회수의 신호보다 앞질러 퍼진다. 꼬리는 신호가 알린 seq 까지만, 권한 다시 판정이 끝난 뒤에
    적용한다(§3.3-101 · 102). 권한 판정이 새 표를 읽게 되면 0016 과 같은 트리거를 그 표에 건다(§3.2-20)
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1612개가 다 돌아야 CI 의 db 잡과 같다.
...
- check · verify · build · e2e 를 한 줄로 이어 백그라운드로 돌리면 **e2e 의 종료 코드 1 은 클립보드 검사 2개 때문**이다 —
  로그 끝의 `210 / 212` 와 실패한 두 항목의 이름을 보고 판단한다
- 커밋 신호(LISTEN)는 **DB 전체**에서 온다 — 다른 검사 파일이 동시에 쓰는 것까지. 신호를 모으는 검사는 자기가 만든 페이지 ·
  워크스페이스 · 사용자 id 로 거르고, 시작점은 표식 신호가 도착한 뒤로 잡는다(change-feed.db.test.ts ①)
- "신호가 늦게 온다"는 기다려서 만들 수 없다 — 실제 LISTEN 을 감싸 붙잡았다 놓는다(collab-server.db.test.ts 의 holdableFeed)
- 0016 트리거 반사실은 `ALTER TABLE … DISABLE TRIGGER` 로 **하나씩** 끄고 그 검사만 돌린 뒤 반드시 켠다 — DB 전역이라 다른 DB 검사 ·
  cf 폴더 반사실과 겹치지 마라. 끝나면 db:verify:schema 의 [10] 이 8개가 켜져 있는지 본다
- 본문을 "받았는가"를 보는 검사는 **본문이 있는 페이지**로 쓴다 — 빈 페이지면 받아도 보이지 않는다(§3.3-103). 옮기기 전 페이지는
  savePageBody 뒤 그 페이지의 doc_update · doc_snapshot 을 지워 흉내 낸다
- 경쟁을 특정 페이지에만 걸려면 표 잠금 대신 **같은 PK 의 행을 커밋하지 않고 넣어** 두고 pg_locks 의 transactionid 대기로
  확인한다 — 다른 검사 파일을 막지 않는다(§3.3-103)
- **알려진 버그 하나를 재는 진단도 같은 함수의 다른 장면까지 넓힌다.** #80 에서 누출 하나를 재려다 네 장면으로 넓히자 "아무도 못
  보는 페이지" 둘이 더 나왔다(§3.3-107). 기존 검사가 틀린 동작을 단언할 수 있다 — 코드의 낡은 주석과 같은 전제면 의심한다
```
