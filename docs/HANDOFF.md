...
**동작하는 것**: 이메일 OTP 로그인 → 워크스페이스 생성 → 이메일 초대 → 수락 → 워크스페이스 진입 → 페이지 생성 → 본문 편집(12종 블록 · 분할/병합 · 중첩 · `/` 메뉴 · 마크다운 · 서식) → 자동 저장 → 하위 페이지 → 페이지 이동 → 휴지통 · 복원 · 영구 삭제 → 멀티 블록 선택 → 블록 이동 · `+` 버튼 · 토글 접기 → 블록 메뉴 · 복사/붙여넣기(중첩 보존) · 이미지(올리기 · 주소 · 드롭 · 붙여넣기) · 끊겨도 사라지지 않는 편집(협업 서버 · IndexedDB 보존본) · 페이지 공유(상속 · 따로 관리하기) · 즐겨찾기 · 최근 방문 → **코멘트 · 인박스**(페이지 코멘트 · 답글 · 해결 · 인박스 읽음/보관 · 안 읽음 배지) → **`Cmd+K` 검색(한국어가 조사를 넘는다 · 권한 필터가 쿼리 안에)** → **풀페이지 데이터베이스**(사이드바에서 만들기 · 속성 6종 추가 · 행 추가 · 셀 편집 · 키보드 이동 · select 옵션 만들기 · "더 보기" · 필터 · 정렬 · 속성 숨기기 · 머리 메뉴) → **내보내기**(페이지 · 표 · 워크스페이스 전체 — Markdown & CSV ZIP · 누르면 개수부터 · 볼 수 없는 페이지는 빠지고 `_export_report.json` 이 센다).

`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 256개 항목을 확인한다**(앱과 협업 서버를 함께 띄운다). 이 PC 에서는 **Chrome 으로** 돌린다 — Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료된다(§6). 기존 클립보드 검사 2개는 이 환경에서 실패한다(§7).

**숫자**: 마이그레이션 21개 / 테이블 46개 / 테스트 1683개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 256개(이 PC 에서 254 통과 — 클립보드 2개) / PR 95개 머지(#96 포함).
...
| 5b | **`@` 자동완성 화면** — 사람 · 페이지 후보(권한으로 거른다 · 상한 20 · IME 조합 중 질의 금지) · 멘션 노드의 이름 그리기(제목 맵처럼 권한으로 거른 맵 · §3.2-22) · 백링크 표시 · e2e | ⬜ |

**1~2조각이 정한 것 — 다음 조각이 지켜야 한다**
...
| `@` 자동완성 · 멘션 노드의 이름이 없다 | 편집기는 멘션 노드를 **빈 원자**로 그린다 — 이름 · 제목 맵이 아직 없다(5b). 검사 · API 로 넣은 멘션만 존재한다 | `editor/schema.ts` `mention` · `pm-adapter.ts` |
| 백링크 화면이 없다 | `listBacklinks` 만 있다. 07 F-07-09 의 "제목 아래 `{#} backlinks`" 는 5b 이후 | `block/link-edges.ts` |
...
다음 작업 — 순서대로:
  ① 코멘트 **5b — `@` 자동완성 화면**(F-07-08): 편집기에서 `@` 를 치면 사람 · 페이지 후보를 띄우고 고르면 멘션 노드를
     넣는다(`userMentionRun` · `pageMentionRun` 과 같은 attr — 표시 텍스트 없이 id 만). 후보는 **권한으로 거른다**(대표적 누출
     지점 — 07 F-07-08 엣지 표) · 상한 20 · 150ms 디바운스 · **한글 IME 조합 중에는 질의하지 않는다**(compositionstart/end).
     멘션 노드의 이름은 하위 페이지 참조의 제목처럼 **권한으로 거른 맵**으로 그린다(§3.2-22 · `pageRefTitles` 와 같은 자리) —
     사람 이름은 멤버 목록, 페이지 제목은 볼 수 있는 것만. `plainText` attr 에 이름을 넣지 마라. 슬래시 메뉴
     (`editor/slash-menu.ts` · `body-editor.tsx` 의 `syncMenu`)가 트리거 · 팝업 · 키 조작의 본이다
  ② 같은 조각이나 그 다음: 백링크 표시(`listBacklinks` 가 있다) · 코멘트 안의 `@`
