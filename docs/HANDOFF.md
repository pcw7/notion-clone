...
**숫자**: 마이그레이션 17개 / 테이블 39개 / 테스트 1672개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 212개(이 PC 에서 210 통과 — 클립보드 2개) / PR 86개 머지(#87 포함).
...
| 6d | **넘기기(화면)** — 본문 편집기가 협업 편집기(`createCollabEditorState`)로 · 서버가 준 Y 상태로 시작(빈 화면 없이) · 하위 페이지는 `at` 으로 · 모르는 참조의 제목 맵 다시 읽기 · 저장 큐(PUT)를 IndexedDB 편집 보존으로 바꾸기를 **한 PR 에서**(§3.3-118 ②) · e2e 가 협업 서버를 띄우고 저장 · 오프라인 · 오류 절을 바인딩으로 · 두 탭 e2e | ⬜ |
...
| **Phase 0 편집기의 낡은 탭은 뺀 참조를 저장마다 다시 보낸다** | 그 사이 휴지통 · 다른 곳으로 간 하위 페이지를 문서에 든 채 저장하면 서버가 빼고 성공을 돌려주지만(§3.2-24), 편집기 문서에는 남아 다음 저장이 또 넣는다 — 로그에 넣고 빼기가 저장마다 쌓인다. 새로고침하면 멈춘다. 편집기가 바인딩으로 바뀌면(6조각) 수선을 받아 사라진다 | `src/app/w/[workspaceId]/[pageId]/body-editor.tsx` |
...
다음 작업 — 순서대로:
  ① 6d(넘기기 — 화면) — §2 CRDT 표의 6d 칸과 "6d 가 지켜야 할 것" 셋(6a · 6b · 6c) · "6조각이 지켜야 할 것" 넷(3a · 5a · 5c · 5d)을 먼저 모아
     읽는다. 본문 편집기를 협업 연결(`openCollabConnection`) + 협업 편집기 상태로 바꾸고, 저장 큐(PUT)를 IndexedDB 보존본 저장소로 바꾸기를
     한 PR 에서(§3.3-118 ②). 페이지 서버 렌더가 Y 상태를 싣는다 · 하위 페이지는 `at` 으로 · 모르는 참조의 제목 맵 · 이유별 화면 · 다시 여는
     동안 편집 막기. e2e 는 협업 서버를 함께 띄우고(앱이 협업 주소를 알아야 한다 — 빌드 때 박히는 `NEXT_PUBLIC_*` 가 아니게) 저장 · 오프라인 ·
     오류 절을 바인딩으로 옮기고 두 탭 절을 더한다
  ② 6e(PUT 본문 저장 걷어내기)
  ③ CRDT 다음은 마스터 문서 §5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이다
  ④ 착수 전에 읽을 것: §2 의 "6조각이 지켜야 할 것" 넷과 "6d 가 지켜야 할 것" 셋 · §3.2-14 · 15 · 17 · 20 · 22 · 25 · §3.3-116~121 · §7 의 협업
     항목 전부 · src/lib/collab/collab-connection.ts · collab-editor.ts · src/app/w/[workspaceId]/[pageId]/body-editor.tsx · page.tsx ·
     src/lib/sync/(지금 저장 큐 — 걷어낼 것) · scripts/e2e-editor.mjs(서버 띄우기 · 저장 · 오프라인 · 오류 절)
