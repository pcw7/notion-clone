...
최종 갱신: 2026-09-12 (PR #34 머지 시점 — **W5-b 편집 확장 완료. 남은 것은 파일·업로드**)

---

## 1. 지금 어디까지 왔나

**Phase 0 (MVP, 6~8주) 중 W1–W4 · W5-a 완료. W5-b 는 편집 확장(선택·이동·메뉴·복붙)까지 끝났고 파일·업로드가 남았다.**
...
| **W5-b** | **편집 확장 · 파일** | **진행 중** | #27(선택) #29(이동) #30(드래그) #31(접힘·e2e) #33(메뉴) #34(복붙) |
| W6 | 내비게이션 · 권한 | | |
| W7 | 검색 | | |
| W8 | DB 코어 · 뷰 | | |

**동작하는 것**: 이메일 OTP 로그인 → 워크스페이스 생성 → 이메일 초대 → 수락 → 워크스페이스 진입 → 페이지 생성 → 본문 편집(12종 블록 · 분할/병합 · 중첩 · `/` 메뉴 · 마크다운 · 서식) → 자동 저장 → 하위 페이지 → 페이지 이동 → 휴지통 · 복원 · 영구 삭제 → 멀티 블록 선택(Esc · Shift+↑↓ · 일괄 삭제/변환/복제/서식) → 블록 이동(`Mod+Shift+↑↓` · `⋮⋮` 드래그) · `+` 버튼 · 토글 접기 → **블록 메뉴(변환·색·복제·링크·삭제) · 복사/붙여넣기(중첩 보존)**.
`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 60개 항목을 확인한다**.

**숫자**: 마이그레이션 8개 / 테이블 24개 / 테스트 671개(CI skip 0) + 브라우저 검증 60개 / PR 34개 머지.
...
**권장 순서**
1. **이미지 업로드** (`F-01-15` + `F-09-08` + `F-12-09`) — **다음.** presigned PUT. 지금은 URL 만 받는다.
   정본이 "3층 분리"로 확정했다(G31): 블록·업로더 UI / API 계약·서명 URL 만료 / 스토리지·용량 한도.
   **`file` 엔티티는 F-12-09 단독 소유**다. 스토리지는 R2 — MinIO 는 금지(CLAUDE.md 절대 제약 1).
2. **낙관적 업데이트 · outbox** (`F-05-04`) — 없으면 네트워크가 끊긴 사이 친 글이 사라진다.
...
**착수 전에 읽을 것**
- `docs/research/12-platform-ux.md` F-12-09(스토리지·용량 한도) · F-12-16
- `docs/research/09-api-integrations.md` F-09-08(서명 URL 만료)
- `docs/research/01-block-editor.md` F-01-15(미디어 블록 — MVP 는 image 만)
...
| 이미지·파일의 참조 카운트 | 복사하면 같은 URL 을 가리키는 블록이 둘이 된다. 정본은 `file(id, url, size, ref_count)` 를 권한다 — 업로드(F-12-09)와 함께 | `block-clipboard.ts` |
...
Phase 0 W1~W4 와 W5-a(페이지 트리)가 끝났고, W5-b 의 편집 확장도 끝났다 —
블록 선택 · 이동(키보드·드래그) · 핸들 메뉴 · 토글 접기 · 복붙(PR #27~#34).
다음은 이미지 업로드다(F-01-15 + F-09-08 + F-12-09). 정본이 3층 분리로
확정했고(G31) file 엔티티는 F-12-09 단독 소유다. 스토리지는 R2 —
MinIO 는 절대 제약 1 이 금지한다. 그다음이 낙관적 업데이트·outbox(F-05-04)와
오류·복구 UX(F-12-16)이고, 그 뒤가 W6(내비게이션·권한)이다.
착수 전에 docs/research/12-platform-ux.md 의 F-12-09 · F-12-16,
09-api-integrations.md 의 F-09-08 을 읽어라.
...
- `npm test` 만 돌리면 DB 테스트가 **조용히 빠진다**(카운트에도 안 잡힌다).
  `npm run db:up` 을 먼저 해야 671개가 다 돈다 — CI 의 db 잡과 같은 수여야 한다
