...
**숫자**: 마이그레이션 17개 / 테이블 39개 / 테스트 1661개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 212개(이 PC 에서 210 통과 — 클립보드 2개) / PR 85개 머지(#86 포함).
...
| 6c | **넘기기 준비 — 브라우저 연결**(화면 없이) — `HocuspocusProvider` 의 수명 · 닫힘 이유별 동작(버리고 다시 열기 · 읽기 전용으로 다시 열기 · 접근 권한 없음 · 로그인 · 잠시 뒤 다시) · 서버가 확인하지 않은 편집의 보존(저장소를 주입받는다)과 그것을 지울 때. 실제 협업 서버에 node 로 붙어 검사한다 | ⬜ |
...
다음 작업 — 순서대로:
  ① 6c(브라우저 연결 모듈 — 화면 없이) — §2 CRDT 표의 6c 칸과 "6c 가 지켜야 할 것 — 6b" · "6조각이 지켜야 할 것" 넷(3a · 5a · 5c · 5d)을
     먼저 모아 읽는다. provider 수명 · 닫힘 이유별 동작 · 버리고 다시 열기 · 서버가 확인하지 않은 편집의 보존과 지울 때를 실제 협업
     서버(`collab-server.db.test.ts` 의 `join` 처럼 node 의 provider)로 검사한다. 보존본을 지울 때(`unsyncedChanges`)는 진단으로 먼저 잰다
  ② 6d(화면 넘기기 + IndexedDB 보존 · e2e 가 협업 서버를 띄운다 · 두 탭) → 6e(PUT 본문 저장 걷어내기)
  ③ CRDT 다음은 마스터 문서 §5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이다
  ④ 착수 전에 읽을 것: §2 의 "6조각이 지켜야 할 것" 넷과 "6c · 6d 가 지켜야 할 것" · §3.2-14 · 15 · 17 · 20 · 22 · 25 · §3.3-116~119 · §7 의
     협업 항목 전부 · src/lib/collab/collab-server.ts(거부 이유 · 닫기) · collab-server.db.test.ts(join) · node_modules/@hocuspocus/provider/src
     (HocuspocusProvider · MessageReceiver) · src/lib/sync/page-sync.ts(지금 저장 큐의 규칙)
