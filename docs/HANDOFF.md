...
**숫자**: 마이그레이션 15개 / 테이블 39개 / 테스트 1586개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 208개(이 PC 에서 206 통과 — 클립보드 2개) / PR 73개 머지(#74 포함).
...
| 항목 | 상태 | 어디 |
|---|---|---|
| **⚠ 휴지통 · 복원 · 영구 삭제 · 이동 명령이 페이지 권한을 보지 않는다** | **5a 에서 발견, 고치지 않았다.** `trashPage` · `restorePage` · `purgePage` · `movePage` 와 그 라우트는 워크스페이스 멤버인지만 본다(`requireWorkspaceSession`). 볼 수 없는 페이지라도 id 를 알면 버리고 옮길 수 있다 — 본문 저장(`edit_content`) · 하위 페이지 생성(`create_child`) · 공유(`manage_perm`)는 본다. 정본 §3.3 의 capability 에는 삭제 · 이동이 따로 없어(`edit_structure` 가 후보) 어느 것으로 볼지부터 정해야 한다. **5b(참여자가 지운 하위 페이지를 휴지통으로) 전에 정한다** — 그 권한 판정이 같은 규칙이어야 한다 | `src/lib/block/trash.ts` · `move-page.ts` · `api/.../trash` · `move` 라우트 |
...
다음 작업은 **휴지통 · 이동 명령의 페이지 권한**(HANDOFF §7 맨 위 — 5a 에서 발견한 구멍)이고, 그다음이 **5b조각 — 정본 프로젝터**다.
  ① 권한: trashPage · restorePage · purgePage · movePage 와 그 라우트가 워크스페이스 멤버인지만 본다 — 볼 수 없는 페이지도 id 로
     버리고 옮길 수 있다. 정본 §3.3 의 capability 에는 삭제 · 이동이 따로 없다(edit_structure 가 후보 · 옮길 곳의 권한도 볼지).
     정본과 권한 도메인 문서(docs/research/06)를 먼저 읽고 정해 §3.2 에 남긴다. 구멍을 재현하는 검사를 먼저 실패시킨다
  ② 5b: 참여자가 지운 살아 있는 하위 페이지를 거부하지 않고 휴지통으로 보낸다(정본 §3.4 X-1 프로젝터 의사코드). 함께 정할 것 —
     그 페이지를 버릴 권한이 없는 참여자(①과 같은 규칙) · page_ref_too_deep(참여자 경로에 거부가 남으면 5d 디바운스를 할 수 없다,
     §3.3-98) · Phase 0 PUT 은 거부를 유지하는가(§3.3-12 낡은 탭 규칙). 5a 의 검사 ⑦(collab-server.db.test.ts)과
     body-write.db.test.ts 의 거부 검사가 그때 뒤집힌다
...
- 시작할 때 `npm run db:up` 을 한 번 돌린다. `npm test` 만 돌리면 DB 테스트가
  **조용히 빠진다**(카운트에도 안 잡힌다). 1586개가 다 돌아야 CI 의 db 잡과 같다.
