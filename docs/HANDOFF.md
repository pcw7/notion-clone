...
**숫자**: 마이그레이션 23개 / 테이블 50개 / 테스트 1775개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 314개(이 PC 에서 312 통과 — 클립보드 2개) / PR 102개 머지(#103 포함).
...
**다음은 5번 — relation (+rollup)** 이다(§8 의 "다음 작업"). 익스포트에 남은 것(잡 · 감사 로그 · 정책 게이트 · HTML · PDF)은 §7.
...
§5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)과 **4번(Board · Group by)이 닫혔다.** 다음은 **5번 — relation (+rollup)** 이다.
...
다음 작업 — 순서대로:
  ① **§5.2 의 4번(Board · Group by)이 닫혔다.** 다음은 **5번 — relation (+rollup)**(F-03-10 · F-03-11 · F-03-07). 마스터 문서:
     *"예약해 둔 `relation_edge` 위에서. rollup 은 v1 에서 **on-read 계산**(1,000행까지 충분)."* 되돌리기 비싼 순서로 자른다 —
     **저장 모양부터**: `relation_edge` 는 0013 에 이미 있다(PK (property_id, from, to) · order_idx · role · owner). 정본 §3.5 의
     불변식 **C2**(relation 은 `page_property_value` 가 아니라 `relation_edge` 가 정본이다 — 셀 EAV 에 쓰지 마라 · CLAUDE.md
     "되돌리기 비싼 4가지")와 03-database-core.md F-03-10(양방향 relation · `config` 의 대상 data_source · 역방향 프로퍼티 쌍) ·
     F-03-11(rollup 집계 함수) 의 엣지 케이스 표를 먼저 읽어라. 조각 제안: 5a = relation 프로퍼티의 config 계약 + 엣지를 쓰는
     서버 명령(셀 쓰기 경로 `row.ts` `prepareCells` 가 relation 을 `unknown_property` 로 막고 있다 — 그 자리를 연다) +
     `properties_cache` 투영(0014 의 R2 트리거가 엣지를 읽는지 확인) → 5b = 화면(행 고르기 팝오버 · 칩) → 5c = rollup
     on-read. **타입을 더하면 컴파일러가 빠진 자리를 짚어 준다**(§3.3-154) — `MVP_PROPERTY_TYPES` 에 넣고 tsc 를 돌려라.
     옵션 타입이 아니므로 `isOptionType` 분기와 섞지 마라
