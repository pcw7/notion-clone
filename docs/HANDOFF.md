...
**숫자**: 마이그레이션 16개 / 테이블 39개 / 테스트 1634개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 212개(이 PC 에서 210 통과 — 클립보드 2개) / PR 82개 머지(#83 포함).
...
5d(투영 디바운스)가 지켜야 할 것 — #83 이 정한 것:
...
다음 작업 — 순서대로:
  ① 5d(투영 디바운스 · projected_seq) — 정본 프로젝터 SLO(p95 < 2s · 쓰기 증폭). 지금은 참여자 update 마다 페이지 행을 잠그고
     본문 전체를 읽어 투영한다. §2 "5d 가 지켜야 할 것" 둘(#82 · #83)을 먼저 읽는다 — 받은 깊이 · 둘 수 있는 하위 페이지는 투영하는
     순간의 행으로 정하고 수선으로 쌓아야 한다. 참여자 경로에 남은 거부 셋(page_ref_forbidden · 모르는 노드가 있을 때의
     page_ref_too_deep · page_ref_unknown)을 쌓기 때 볼지 먼저 정한다. 정본 §3.4 의 프로젝터 의사코드 · projected_seq 를 읽는다
  ② 6조각(에디터 바인딩) — 지켜야 할 것이 §2 의 3a · 5a · 5c 항목에 쌓여 있다(참조 제목 맵 · 되돌리기가 되살린 참조 포함)
  ③ 착수 전에 읽을 것: 정본 §3.4 · §3.7 · §7 "투영이 쓰기와 같은 트랜잭션에서 동기로 돈다" · §3.2-17 · §3.2-19 · §3.2-23 · §3.2-24 ·
     src/lib/block/body-write.ts(openPageBody · finish · appendDocUpdate) · src/lib/collab/collab-server.ts(beforeSync)
