...
**숫자**: 마이그레이션 17개 / 테이블 39개 / 테스트 1653개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 212개(이 PC 에서 210 통과 — 클립보드 2개) / PR 84개 머지(#85 포함).
...
| 6b | **편집기를 협업 서버에 붙인다(넘기기)** — `HocuspocusProvider` · 서버가 준 Y 상태로 시작(빈 화면 없이) · 닫힘 이유별 처리(버리고 다시 열기 · 읽기 전용 · 접근 권한 없음 · 로그인) · 하위 페이지를 만들 때 참조를 서버가 커서 자리에 넣기(로컬에 넣으면 둘이 된다) · 새 참조의 제목 맵 채우기 · e2e 가 협업 서버를 띄우고 저장 절을 바인딩으로 · 두 탭 e2e | ⬜ |
| 6c | **오프라인 보존** — y-indexeddb(MIT)로 닫았다 연 탭에도 편집이 남는다. 본문 저장 큐(IndexedDB outbox)를 걷어낸다 · 버린 로컬 문서의 보존본도 지운다 | ⬜ |
| 6d | **PUT 본문 저장 걷어내기** — 편집기가 쓰지 않게 된 통째 저장(§7 "Phase 0 편집기의 PUT 본문 저장은 Y.Doc 을 통째로 받은 문서로 맞춘다")을 API 에서 어떻게 할지(없애기 · 명령으로 좁히기) 정하고, e2e 준비를 명령으로 옮긴다 | ⬜ |
...
- 편집기 · 협업 서버 어디에도 구조 위반을 고쳐 쓰는 appendTransaction 을 두지 않는다(§3.2-14). 수선 전 위반 문서에서 **키**는 삼키지만
  마우스 · 드래그 · 붙여넣기 · 메뉴가 부르는 명령은 삼키지 않는다 — 6b 의 e2e 에서 본다(§7)
...
| **수선 전까지 에디터 문서가 스키마를 어긴다** | 바인딩은 Y.Doc 을 그대로 비추므로 수선이 도착하기 전까지 한 블록에 내용 줄 둘 · 루트 그룹 둘 같은 문서를 들고 있다. 그 자리를 건드리는 스텝은 ProseMirror 가 거부한다(던진다). **키는 6a 에서 막았다** — 협업 편집기는 명령이 던지면 키를 삼킨다(진단: 타입이 동시에 바뀐 블록 뒤의 Backspace 가 던졌다 · `collab-editor.test.ts` ④). **남은 것**: 마우스 · 드래그 · 붙여넣기 · 블록 메뉴가 부르는 명령은 삼키지 않는다. `pm-blocks.ts` 는 첫 루트 그룹만 평탄화하고, 블록 선택 · 핸들 · 접힘은 이 모양을 보지 않았다 — 6b 의 e2e 에서 본다 | `src/lib/collab/collab-editor.ts` · `src/lib/editor/pm-blocks.ts` |
...
다음 작업 — 순서대로:
  ① 6b(편집기를 협업 서버에 붙인다 — 넘기기) — §2 CRDT 표의 6b 칸과 "6b 가 지켜야 할 것 — 6a" · "6조각이 지켜야 할 것" 넷(3a · 5a · 5c · 5d)을
     먼저 모아 읽는다. 넘기기 전에 정할 것: 서버가 준 Y 상태로 시작하는 길(페이지 서버 렌더가 `loadDocState` 의 update 를 싣는가) · 하위
     페이지 생성이 참조를 커서 자리에 넣는 길(`createPage` 가 자리를 받는다 — 로컬에 넣으면 참조가 둘이 된다) · 새 참조의 제목 맵 · 닫힘
     이유별 화면 · 6c(오프라인 보존) 전까지 저장 큐를 남길지(§3.3-116 ③). e2e 는 협업 서버를 함께 띄운다
  ② 6c(오프라인 보존 · y-indexeddb) → 6d(PUT 본문 저장 걷어내기)
  ③ CRDT 다음은 마스터 문서 §5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이다
  ④ 착수 전에 읽을 것: §2 의 "6조각이 지켜야 할 것" 넷과 "6b 가 지켜야 할 것" · §3.2-14 · 15 · 17 · 20 · 22 · 25 · §3.3-116 · 117 · §7 의 협업
     항목 전부 · src/lib/collab/collab-editor.ts · src/app/w/[workspaceId]/[pageId]/body-editor.tsx · scripts/e2e-editor.mjs(서버 띄우기 · 저장 절)
