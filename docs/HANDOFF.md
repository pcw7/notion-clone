...
`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 218개 항목을 확인한다**(앱과 협업 서버를 함께 띄운다). 이 PC 에서는 **Chrome 으로** 돌린다 — Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료된다(§6). 기존 클립보드 검사 2개는 이 환경에서 실패한다(§7).

**숫자**: 마이그레이션 17개 / 테이블 39개 / 테스트 1629개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 218개(이 PC 에서 216 통과 — 클립보드 2개) / PR 88개 머지(#89 포함).
...
| 6e | **PUT 본문 저장 걷어내기** — 편집기가 쓰지 않게 된 통째 저장(§7 "Phase 0 편집기의 PUT 본문 저장은 Y.Doc 을 통째로 받은 문서로 맞춘다")을 API 에서 어떻게 할지(없애기 · 명령으로 좁히기) 정하고, e2e 준비를 명령으로 옮긴다 | ⬜ |
...
다음 작업 — 순서대로:
  ① 6e(PUT 본문 저장 걷어내기) — §2 CRDT 표의 6e 칸과 "6e 가 지켜야 할 것 — 6d" 를 먼저 읽는다. 본문 저장(PUT)을 부르는 곳은 이제
     검사 · 도구뿐이다(e2e 준비 · `savedShape` · 익스포트 검사). 라우트를 없앨지 · 명령으로 좁힐지 정하고 그 준비를 옮긴다.
     `loadPageBody`(행으로 읽기)는 익스포트 · 검색 · 검사가 아직 쓴다 — 화면은 쓰지 않는다
  ② 그다음은 CRDT 를 닫고 §5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)
  ③ CRDT 다음은 마스터 문서 §5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이다
  ④ 착수 전에 읽을 것: §2 의 "6조각이 지켜야 할 것" 넷과 "6e 가 지켜야 할 것" · §3.2-14 · 15 · 17 · 20 · 22 · 25 · §3.3-116~123 · §7 의 협업
     항목 전부 · src/app/api/workspaces/[workspaceId]/pages/[pageId]/body/route.ts · src/lib/block/save-page-body.ts ·
     scripts/e2e-editor.mjs(PUT 로 준비하는 자리) · src/lib/export/(행으로 읽는 곳)
