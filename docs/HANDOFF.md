...
**동작하는 것**: 이메일 OTP 로그인 → 워크스페이스 생성 → 이메일 초대 → 수락 → 워크스페이스 진입 → 페이지 생성 → 본문 편집(12종 블록 · 분할/병합 · 중첩 · `/` 메뉴 · 마크다운 · 서식) → 자동 저장 → 하위 페이지 → 페이지 이동 → 휴지통 · 복원 · 영구 삭제 → 멀티 블록 선택 → 블록 이동 · `+` 버튼 · 토글 접기 → 블록 메뉴 · 복사/붙여넣기(중첩 보존) · 이미지(올리기 · 주소 · 드롭 · 붙여넣기) · 끊겨도 사라지지 않는 편집(협업 서버 · IndexedDB 보존본) · 페이지 공유(상속 · 따로 관리하기) · 즐겨찾기 · 최근 방문 → **코멘트 · 멘션 · 인박스**(페이지 · 글자 범위 코멘트 · 답글 · 해결 · `@` 멘션 · 백링크 · 인박스 읽음/보관 · 안 읽음 배지) → **`Cmd+K` 검색(한국어가 조사를 넘는다 · 권한 필터가 쿼리 안에)** → **풀페이지 데이터베이스**(사이드바에서 만들기 · 속성 7종 추가(상태 포함 — 새 행은 기본 옵션을 받는다) · 행 추가 · 셀 편집 · 키보드 이동 · select 옵션 만들기 · "더 보기" · 필터 · 정렬 · 속성 숨기기 · 머리 메뉴) → **보드 뷰**(탭에서 만들기 · 열 = 그룹 · 카드를 끌어 값과 순서를 바꾸기 · 열 머리 `+` · 그룹 숨기기 · 그룹 기준) → **내보내기**(페이지 · 표 · 워크스페이스 전체 — Markdown & CSV ZIP · 누르면 개수부터 · 볼 수 없는 페이지는 빠지고 `_export_report.json` 이 센다).

`npm run dev` 로 실제로 눌러볼 수 있고, **`npm run e2e` 가 실제 브라우저로 306개 항목을 확인한다**(앱과 협업 서버를 함께 띄운다). 이 PC 에서는 **Chrome 으로** 돌린다 — Edge 153 헤드리스는 진짜 `Ctrl+C` 에서 종료된다(§6). 기존 클립보드 검사 2개는 이 환경에서 실패한다(§7).

**숫자**: 마이그레이션 23개 / 테이블 50개 / 테스트 1761개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 306개(이 PC 에서 304 통과 — 클립보드 2개) / PR 100개 머지(#101 포함).
...
공개 링크(F-06-06) · 인라인 DB 는 §5.2 표에 없다. §5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이 닫혔고, **지금은 4번 —
Board · Group by** 를 조각으로 진행 중이다(아래 보드 조각 표). 익스포트에 남은 것(잡 · 감사 로그 · 정책 게이트 · HTML · PDF)은 §7.
...
→ 풀페이지 데이터베이스(표 · 보드 · 속성 7종(상태 포함) · 셀 편집 · 필터 · 정렬 · 속성 숨기기 · 카드 드래그)
...
§5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이 닫혔고, 지금은 **4번 — Board · Group by** 를 조각으로 진행 중이다.
HANDOFF §2 의 **보드 조각 표**와 "보드 4a 가 정한 것"을 먼저 읽어라. 코멘트 조각 표 · CRDT 조각 표는 그쪽을 건드릴 때 읽는다.
...
다음 작업 — 순서대로:
  ① **보드 4a · 4b · 4c-1(status)이 끝났다.** 다음은 **4c-2 — List 뷰(F-04-04)**: **표의 축약 렌더러**다 — 별도 컴포넌트
     트리를 만들지 마라(F-04-04: *"table view 렌더러에 `variant='list'` 를 두어 셀 컴포넌트를 재사용하고 그리드 레이아웃만
     교체한다. 별도 트리를 만들면 셀 에디터를 두 벌 유지하게 된다"*). 서버는 이미 받는다(`MVP_VIEW_TYPES` 에 list ·
     0022 의 CHECK). 할 일: `view-tabs.tsx` 의 CREATABLE 에 list 를 열고, `page.tsx` 가 list 면 `DatabaseTable` 에 variant 를
     준다 — 제목은 왼쪽, 보이는 속성은 오른쪽 배지(`CellDisplay` 재사용), 머리 행 없음, 표시 속성 토글은 기존 "속성"
     패널(`view_property.visible`) 그대로. 키보드 내비게이션(`grid-nav.ts`)을 list 에서 어떻게 할지 먼저 정한다(F-04-04 는
     말하지 않는다). 이것으로 §5.2 의 4번이 닫힌다
