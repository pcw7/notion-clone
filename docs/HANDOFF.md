...
**숫자**: 마이그레이션 17개 / 테이블 39개 / 테스트 1629개(CI skip 0 · 이 PC 는 외부 도구 검사가 셸에 따라 1~2개 skip — PowerShell 은 unzip, Git Bash 는 bsdtar 를 못 찾는다) + 브라우저 검증 216개(이 PC 에서 214 통과 — 클립보드 2개) / PR 89개 머지(#90 포함).
...
공개 링크(F-06-06) · 인라인 DB 는 §5.2 표에 없다 — **다음은 3번(코멘트 · 멘션 · 알림 · 인박스)**이다. 익스포트에 남은 것(잡 · 감사 로그 · 정책 게이트 · HTML · PDF)은 §7.
...
다음 작업 — 순서대로:
  ① **CRDT 가 닫혔다.** 다음은 마스터 문서 §5.2 의 3번 — 코멘트 · 멘션 · 알림 · 인박스(F-08-* · F-11-*). 정본 §3.8(코멘트 ·
     스레드)과 §3.10(알림)을 먼저 읽고, 조각을 되돌리기 비싼 순서로 자른다. 본문 범위를 가리키는 코멘트의 자리는 Y.Doc 이
     정본이다 — §2 의 "CRDT 다음이 지켜야 할 것"을 먼저 읽는다
  ② 남은 것들: 익스포트의 잡 · 감사 로그 · HTML(§7) · awareness(커서 · 참여자 목록) · 공개 링크(F-06-06) · 인라인 DB
  ③ CRDT 다음은 마스터 문서 §5.2 의 3번(코멘트 · 멘션 · 알림 · 인박스)이다
  ④ 착수 전에 읽을 것: §2 의 "CRDT 다음이 지켜야 할 것" · 정본 §3.8 · §3.10 · 마스터 문서 §5.2 · §3.2-14~25(CRDT 판결) ·
     §3.3-116~124 · §7 의 협업 항목 전부 · src/lib/collab/(본문을 쓰는 유일한 길) · src/lib/block/body-write.ts
